...
function ajaxValidateProcess(){
	...
	var uri = "/IOSWEBINT1Policy/ManualServlet";
...
/*-- Call AJAX for validate master before save --*/
function ajaxValidateMaster(){
	var formName = "masterForm";
	var uri = "/IOSWEBINT1Policy/ManualServlet";
	var dataString = "className=com.master.manual.ValidateMasterManualClass";
	dataString += "&action=insertMany";
	dataString += "&handleForm=Y";
	dataString += sweepAjaxForm(formName);
...
function textBoxToDatePicker(textBoxName){
	// paint datePicker
	jQuery("[name='"+textBoxName+"']").after('&nbsp;<IMG id="'+textBoxName+'_IMG" style="CURSOR:hand" alt="" src="/IOSWEBINT1Policy/images/calendar_new.gif" />');
