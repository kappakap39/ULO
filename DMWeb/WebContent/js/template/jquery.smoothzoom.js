...
(function ($) {
	...
	function zoomer($image, params) {
		...
		var op = $.extend({}, {
			...
			button_ICON_IMAGE		: 'images/icons.png',
			button_AUTO_HIDE		: 'NO',
...
//					_y = tY;
			...
			if (rA == rF) {
				if (buttons[1].$ob.css('opacity') > .5) {
					if (rA >= rF) {
						if (pan_limit && mouse_drag && !_moveCursor) $image.css('cursor', 'move'), $hitArea.css('cursor', 'move');
...
//			Rawi Fix For ie9 20111229
			
//			if( buttonNumber == 2 ||  buttonNumber == 4 || buttonNumber == 7 || buttonNumber == 9 ||buttonNumber == 11 || buttonNumber == 13 ){
//				topButtonPx = _r + 3;				
//			}else if(buttonNumber == 3  || buttonNumber == 5){
//				topButtonPx = _r + 5;	
//			}else if (buttonNumber == 6  || buttonNumber == 8||buttonNumber == 10  || buttonNumber == 12||buttonNumber == 14  || buttonNumber == 15) {
//				topButtonPx = _r + 1;
//			}else{
//				topButtonPx = _h - _r - 8;
//			}			
			...
			$('<div class="bgh' + _name + '" style = "top:' + (topButtonPx) + 'px; left:' + _r + 'px"></div>').appendTo(el);			
