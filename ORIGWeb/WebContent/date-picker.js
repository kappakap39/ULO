...
Calendar.prototype.show = function() {
	...
	var yearThai = parseInt(this.gYear);
	this.wwriteA(this.gMonthName + " " + yearThai);
	this.wwriteA("</B><BR>");
	...
	this.wwrite("[<A HREF=\"" +"javascript:window.opener.Build(" + 
		"'" + this.gReturnItem + "', '" + this.gMonth + "', '" + (parseInt(this.gYear)-1) + "', '" + this.gFormat + "'" +");" +"\"><<<\/A>]</TD><TD ALIGN=center>");
	this.wwrite("[<A HREF=\"" +"javascript:window.opener.Build(" +"'" + this.gReturnItem + "', '" + prevMM + "', '" + prevYYYY + "', '" + this.gFormat + "'" +");" +
		"\"><<\/A>]</TD><TD ALIGN=center>");
	this.wwrite("[<A HREF=\"javascript:window.print();\">Print</A>]</TD><TD ALIGN=center>");
	this.wwrite("[<A HREF=\"" +"javascript:window.opener.Build(" +"'" + this.gReturnItem + "', '" + nextMM + "', '" + nextYYYY + "', '" + this.gFormat + "'" +");" +
		"\">><\/A>]</TD><TD ALIGN=center>");
	this.wwrite("[<A HREF=\"" +"javascript:window.opener.Build(" +
		"'" + this.gReturnItem + "', '" + this.gMonth + "', '" + (parseInt(this.gYear)+1) + "', '" + this.gFormat + "'" +");" +"\">>><\/A>]</TD></TR></TABLE><BR>");
...
Calendar.prototype.showY = function() {
	...
	for (i=11; i>=0; i--) {
		...
		else if (isNav)	this.wwrite("</LAYER>");
	}
...
Calendar.prototype.cal_data = function() {
	...
	*/
	...
	for (var j=vFirstDay; j<7; j++) {
		...
		"onClick=\"javascript:opener.displayDateField('"+this.gReturnItem+"','"+this.format_data(vDay)+"');window.close();\">" + 
		this.format_day(vDay) + "</A>" + "</FONT></TD>";
		vDay=vDay + 1;
	}
	vCode = vCode + "</TR>"; 
	for (var k=2; k<7; k++) {
		...
		for (j=0; j<7; j++) {
			vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + "><FONT SIZE='2' FACE='" + fontface + "'>" + 
				"<A HREF='#' "+"onClick=\"javascript:opener.displayDateField('"+this.gReturnItem+"','"+this.format_data(vDay)+"');window.close();\">" + 
				this.format_day(vDay) + "</A>" + "</FONT></TD>";
			vDay=vDay + 1;
			if (vDay > vLastDay) {
				vOnLastDay = 1;break;
	...
	for (var m=1; m<(7-j); m++) {
		if (this.gYearly)
			vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j+m) + "><FONT SIZE='2' FACE='" + fontface + "' COLOR='gray'> </FONT></TD>";
		else vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j+m) + 
...
function show_calendar() {
	...
	*/
	...
	var tempVal = $("[name='edit-cc-lastpayment-date']").val();
	...
	vWinCal = window.open("", "Calendar","width=250,height=250,status=no,resizable=no,top=300,left=500");
	vWinCal.opener = self;
	ggWinCal = vWinCal;
	//alert(p_month+"-"+p_year+"-"+p_format);
	if(tempVal!=""){
		var tempSplit = tempVal.split("/");
		p_month = tempSplit[1]-1;
		p_year = parseInt(tempSplit[2]);
		p_year = p_year.toString();
	}
	Build(p_item, p_month, p_year, p_format);
}
function displayDateField(elementID,value){	
	$('#'+elementID).val(value);
	$('#'+elementID).focus();
}