...
<TABLE height=1 cellSpacing=0 cellPadding=0 width="100%" bgColor=#FFFFFF border=0 align="center">   
    ...
    <TR>
		<TD align="right">
			<INPUT class="button_text" onclick="javascript:mandatoryField('<%=customerType %>', '<%=userRole %>', '<%=popupForm.getFormID() %>', 'Y')" type="button" value=" <%=MessageResourceUtil.getTextDescription(request, "SAVE") %> " name="saveGuarantorBtn" <%=disabledBtn %>>
			<INPUT class="button_text" onclick="javascript:closeGuarantor()" type="button" value=" <%=MessageResourceUtil.getTextDescription(request, "CLOSE") %> " name="closeAppBtn"> 
		</TD>
	</TR>
	<TR><TD height="10"></TD></TR>
</TABLE>
