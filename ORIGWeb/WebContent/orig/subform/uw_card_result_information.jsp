...
<% 	
	...
	String displayMode = formUtil.getDisplayMode("UW_CARD_RESULT_SUBFORM", ORIGUser.getRoles(), applicationDataM.getJobState(), ORIGForm.getFormID(), searchType);	
...
//	String displayMode = formUtil.getDisplayMode("", ORIGUser.getRoles(), applicationDataM.getApplicationStatus(), ORIGForm.getFormID());	
	...
	BigDecimal finalCreditLimit = applicationDataM.getFinalCreditLimit();
		...
		<td>
			<table cellpadding="0" cellspacing="1" width="100%" align="center">
				<%if(! ( OrigConstant.ORIG_FLAG_Y.equalsIgnoreCase( applicationDataM.getDrawDownFlag())||ORIGWFConstant.JobState.UW_NEW_PROPOSAL_STATE.equals(applicationDataM.getJobState())||ORIGWFConstant.JobState.UW_PROPOSAL_PENDING_STATE.equals(applicationDataM.getJobState()))) {%>
 			    <tr> 
					<td class="text-header-action1" width="20%"><%=MessageResourceUtil.getTextDescription(request, "POLICY_VERSION") %></td>
					<td class="text-header-action1" width="80%"><%=ORIGDisplayFormatUtil.displayHTML(applicationDataM.getPolicyVersion()) %></td>
				</tr>
				<tr>
					<td class="text-header-action1" width="20%"><%=MessageResourceUtil.getTextDescription(request, "APPLICATION_SCORE") %><Font color="#ff0000"><%=ORIGDisplayFormatUtil.getMandatory_ORIG(personalInfoDataM.getCustomerType(),ORIGUser.getRoles(),"UW_RESULT_SUBFORM","application_score")%></Font> :</td>
					<td class="text-header-action1" width="80%">
						<table cellpadding="0" cellspacing="1" width="200" align="left">
						<tr><td width="50">
							<%=ORIGDisplayFormatUtil.displayInputTagScriptAction(applicationDataM.getScorringResult(),displayMode,"","application_score","inputformnew"," readonly ","50") %></td>
						<td width="150">
					    	<%=ORIGDisplayFormatUtil.displayButtonTagScriptAction("Calculate Score",displayMode,"button", "","smallbtn2"," onClick=\"mandatoryScoreField(this.form)\"","") %></td>
					    </tr>
					    </table>
					</td>
				</tr>
				<%}%>
				<tr>
					<td class="text-header-action1"><%=MessageResourceUtil.getTextDescription(request, "FINAL_CREDIT_LIMIT") %> :</td>
					<td class="text-header-action1"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(ORIGDisplayFormatUtil.displayCommaNumber(finalCreditLimit),displayMode,"","final_credit_limit","inputformnewCurrency","onblur=\"javascript:addComma(this)\" onKeyPress=\"isNumberOnKeyPress(this)\" onkeyup=\"isNumberOnkeyUp(this)\" onfocus=\"removeCommaField(this)\" ","50") %></td>
				</tr>
			</table>
		</td>
	</tr>
	...
	<tr>
		<td>
			<table cellpadding="" cellspacing="1" width="100%" align="center">
				...
				<%if(!proposalFlag){%>
				<tr>
					<td class="text-header-action1" width="20%" rowspan="8"><%=MessageResourceUtil.getTextDescription(request, "DECISION") %><Font color="#ff0000"><%=ORIGDisplayFormatUtil.getMandatory_ORIG(personalInfoDataM.getCustomerType(),ORIGUser.getRoles(),"UW_RESULT_SUBFORM","decision_uw")%></Font> :</td>
					<td class="text-header-action1" width="80%"  colspan="2"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.APPROVE,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:clearEscalateGroup();checkApproveApp();\"") %> <%=MessageResourceUtil.getTextDescription(request, "APPROVE") %> </td>
				</tr>
				<tr>
					<td class="text-header-action1" colspan="2"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.REJECT,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:getReason(this.value,'reason_uw');clearEscalateGroup();\"") %> <%=MessageResourceUtil.getTextDescription(request, "REJECT") %> </td>
				</tr>
				<tr>
					<td class="text-header-action1" colspan="2"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.CANCEL,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:getReason(this.value,'reason_uw');clearEscalateGroup();\"") %> <%=MessageResourceUtil.getTextDescription(request, "CANCEL") %> </td>
				</tr>
				<tr>
					<td class="text-header-action1" colspan="2"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.UW_PENDING,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:clearEscalateGroup();checkExceptionApp();getReason(this.value,'reason_uw');\"") %> <%=MessageResourceUtil.getTextDescription(request, "PENDING") %> </td>
				</tr>
				<% 
				...
				%>
				<tr>
					<td class="text-header-action1" width="20%"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.ESCALATE,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:getEscalateGroup();\" "+disabledEscalate) %> <%=MessageResourceUtil.getTextDescription(request, "ESCALATE") %> </td>
					<td id="EscalateGroup" class="inputCol" ></td>
				</tr>
				<tr>
					<td class="text-header-action1" colspan="2"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.CONDITIONAL_APPROVE,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:clearEscalateGroup();checkExceptionApp();getReason(this.value,'reason_uw');\"") %> <%=MessageResourceUtil.getTextDescription(request, "CONDITIONAL_APPROVE") %> </td>
				</tr>
				<tr>
					<td class="text-header-action1"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.APPROVE_WITH_EXCEPTION,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:clearEscalateGroup();checkApproveApp();\"") %> <%=MessageResourceUtil.getTextDescription(request, "SEND_TO_XCMR") %></td>
					<td id="CMRExceptionMSG" class="inputCol"></td>
				</tr>
				<tr>
					<td class="text-header-action1"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(OrigConstant.ApplicationDecision.SENDTO_POLICY_EXCEPTION,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:clearEscalateGroup();checkApproveApp();\"") %> <%=MessageResourceUtil.getTextDescription(request, "SEND_TO_XUW") %></td>
					<td id="PolicyExceptionMSG" class="inputCol"></td>					
				</tr>
				<%}else{ %>
				<tr>
					<td class="text-header-action1" width="20%" rowspan="6"><%=MessageResourceUtil.getTextDescription(request, "DECISION") %> :</td>
					<td class="text-header-action1" width="80%"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.APPROVE_PROPOSAL,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:getReason(this.value,'reason_uw');\" ") %> <%=MessageResourceUtil.getTextDescription(request, "APPROVE") %> </td>
				</tr>
				<tr>
					<td class="text-header-action1"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.REJECT_PROPOSAL,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:getReason(this.value,'reason_uw');\"") %> <%=MessageResourceUtil.getTextDescription(request, "REJECT") %> </td>
				</tr>
				<tr>
					<td class="text-header-action1"><%=ORIGDisplayFormatUtil.displayRadioTagScriptAction(ORIGWFConstant.ApplicationDecision.PENDING_PROPOSAL,displayMode,"decision_uw",applicationDataM.getUwDecision(),"","","onClick=\"javascript:getReason(this.value,'reason_uw');\"") %> <%=MessageResourceUtil.getTextDescription(request, "PENDING") %> </td>
				</tr>
				<%} %>
			</table>
		</td>
		...
		<td>
			<table cellpadding="" cellspacing="1" width="100%" align="center">
				<tr>
					...
					<td class="text-header-action1" width="80%"><div id="ReasonIDUW" class="inputformnew2" style="OVERFLOW: scroll;width: 400px; height: 250px;" <%=disableItem%>>
							<%
								if(reasonVect != null){
				...
				<tr>
					<td class="text-header-action1"><%=MessageResourceUtil.getTextDescription(request, "DECISION_REASON") %> :</td>
					<td class="text-header-action1"><%--=ORIGDisplayFormatUtil.displayInputTagScriptAction(applicationDataM.getUwDecisionReason(),displayMode,"75","decision_reason","textbox","","200") --%>
