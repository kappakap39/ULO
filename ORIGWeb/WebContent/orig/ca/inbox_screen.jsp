...
<%  org.apache.log4j.Logger log=org.apache.log4j.Logger.getLogger("orig/ca/inbox_screen.jsp");
...
%>
<script type="text/javascript" src="orig/js/inbox/pl/ca.inbox.js"></script>
			...
			<div class="PanelThird">
				<table class="TableFrame">
					<tr class="Header">
						...
						<td colspan="10"><%=PLMessageResourceUtil.getTextDescription(request, "APPLICATION_INFORMATION")%></td>
						<td colspan="2"><%=PLMessageResourceUtil.getTextDescription(request, "APPLICATION_TIME")%></td>
					</tr>
					<tr class="Header">
						<td width="7%"><%=PLMessageResourceUtil.getTextDescription(request, "DATE_RECEIVE_WORK")%></td>
						<td width="7%"><%=PLMessageResourceUtil.getTextDescription(request, "REMAINING_TIME")%></td>
						...
						<td width="8%"><%=PLMessageResourceUtil.getTextDescription(request, "APP_STATUS")%></td>
						<td width="5%"><%=PLMessageResourceUtil.getTextDescription(request, "VETO")%></td>
						<td width="7%"><%=PLMessageResourceUtil.getTextDescription(request, "IN_DECREASE_FLAG")%></td>
						<td width="9%"><%=PLMessageResourceUtil.getTextDescription(request, "NAME_SURNAME")%></td>
						<td width="8%"><%=PLMessageResourceUtil.getTextDescription(request, "ID_NO")%></td>
						...
						<td width="7%"><%=PLMessageResourceUtil.getTextDescription(request, "REMAINING_ALL_TIME")%></td>
					</tr>
			 <%if(!ORIGUtility.isEmptyVector(jobVect)) { %>
			 	<%for(int i = 0; i < jobVect.size() ; i++){
			 		...
			 		if(OrigConstant.FLAG_Y.equals(jobModel.getInfoICDCFlag())){
			 			styleTr = "ResultBlue";
			 		}
			 	%>
		 			...
		 			<tr class="Result-Obj <%=styleTr%>" id="<%=jobModel.getAppRecordID()%>">
						...
						<td><span class="<%=spanClass%>"><%=HTMLRenderUtil.displayTypeColor(jobModel.getInfoJobType())%></span></td>
		 				...
		 				<td><span class="<%=spanClass%>"><%=HTMLRenderUtil.displayFlagYesNo(jobModel.getInfoICDCFlag()) %></span></td>
