$(document).ready(function(){
	...
	$("input[name=BRANCH_TYPE_BOX]").change(function(){
		if($("input[name=BRANCH_TYPE_BOX]").is(":checked")){
			...
			$("#BRANCH_ZONE_TRANSFER .removeAll").click();
			$("#BRANCH_REGION_TRANSFER *").prop("disabled",true);
			$("#BRANCH_ZONE_TRANSFER *").prop("disabled",true);
		}
	});
...
function SLA_REPORT_GENERATE_BUTTONActionJS(){
	var dateFromArray = $("input[name=DATE_FROM_CALENDAR]").val().split("/");
	var dateFrom = dateFromArray[2]+"-"+dateFromArray[1]+"-"+dateFromArray[0];
	var dateFromObj = new Date(dateFrom);
	
	var dateToArray = $("input[name=DATE_TO_CALENDAR]").val().split("/");
	var dateTo = dateToArray[2]+"-"+dateToArray[1]+"-"+dateToArray[0];
	var dateToObj = new Date(dateTo);

		
	var validateId = VALIDATE_SLA_REPORT;
	var $data = $('#SLA_REPORT *').serialize();
...
function getOperatingReport(){
	...
	if(BRANCH_REGION_TRANSFER==null){
		if($("input[name=BRANCH_TYPE_BOX]").is(":checked")){
			$("#BRANCH_REGION_TRANSFER #select2 option").val("ALL");
			data = data+"&BRANCH_REGION_TRANSFER=ALL";
		}
	}else{
	...
	if(BRANCH_ZONE_TRANSFER==null){
		if($("input[name=BRANCH_TYPE_BOX]").is(":checked")){
			$("#BRANCH_ZONE_TRANSFER #select2 option").val("ALL");
			data = data+"&BRANCH_ZONE_TRANSFER=ALL";
		}
	}else{
	...
	if(DSA_EXPAND_ZONE_TRANSFER==null){
		if($("input[name=DSA_TYPE_BOX]").is(":checked")){
			$("#DSA_EXPAND_ZONE_TRANSFER #select2 option").val("ALL");
			data = data+"&DSA_EXPAND_ZONE_TRANSFER=ALL";
		}
	}else{
	...
	if(NBD_ZONE_TRANSFER==null){
		if($("input[name=NBD_TYPE_BOX]").is(":checked")){
			$("#NBD_ZONE_TRANSFER #select2 option").val("ALL");
			data = data+"&NBD_ZONE_TRANSFER=ALL";
		}
	}else{
...
function displayJSON(data){
	...
	var GENERATE_BUTTON_FLAG = $("[name='GENERATE_BUTTON_FLAG']").val();
	if(false == obj.isGenerateReport && FLAG_YES == GENERATE_BUTTON_FLAG){
...
function SLA_REPORT_RESET_BUTTONActionJS(){
	$("form").trigger("reset");
	$("#BRANCH_REGION_TRANSFER *").prop("disabled",true);
	$("#BRANCH_ZONE_TRANSFER *").prop("disabled",true);
	$("#DSA_EXPAND_ZONE_TRANSFER *").prop("disabled",true);
...
//	focusElementActionJS("PROJECT_NO_BOX", '');
}
