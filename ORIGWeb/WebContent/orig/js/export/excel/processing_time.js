...
function PT_REPORT_GENERATE_BUTTONActionJS(){
	...
	if(dateFromObj>dateToObj){
		alertBox(ERROE_DATE_MORE_THAN_CURRENT_DATE);
	}else{
		var validateId = VALIDATE_PT_REPORT;
		var $data = $('#PT_REPORT *').serialize();
...
function PT_REPORT_RESET_BUTTONActionJS(){
	$("form").trigger("reset");
	$("#BRANCH_REGION_TRANSFER *").prop("disabled",true);
	$("#BRANCH_ZONE_TRANSFER *").prop("disabled",true);
	$("#DSA_EXPAND_ZONE_TRANSFER *").prop("disabled",true);
...
//	alert( $( "select" ).length);
	for (var i = 0; i < $( "select" ).length; i++) {
		$( "select" )[i].selectize.setValue("",true);
	}
}
