                        ...
                        <tr class="sidebar10"> <td align="center">
							<table width="100%" align=center cellpadding="0" cellspacing="1" border="0">
			 					<tr>
								 	...
								 	<td class="inputCol" width="25%"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_XCMR_DATAM.getCitizenID(), displayMode, "", "citizen_id", "textbox", "", "")%></td>
								 	<td class="textColS" width="25%"><%=MessageResourceUtil.getTextDescription(request, "APPLICATION_NO") %> :</td>
								 	<td class="inputCol" width="25%"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_XCMR_DATAM.getApplicationNo(), displayMode, "", "application_no", "textbox", "", "")%></td>
								 </tr>
								 <tr>
								 	<td class="textColS" ><%=MessageResourceUtil.getTextDescription(request, "THAI_FNAME") %> :</td>
								 	<td class="inputCol" ><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_XCMR_DATAM.getCustomerName(), displayMode, "", "company_name", "textbox", "", "")%></td>
								 	<td class="textColS" ><%=MessageResourceUtil.getTextDescription(request, "THAI_LNAME") %> :</td>
								 	<td class="inputCol" ><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_XCMR_DATAM.getCustomerLName(), displayMode, "", "lastname", "textbox", "", "")%></td>			 				 	
								 </tr>
								 <tr>
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "VEHICLE_REGIS_NO") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_XCMR_DATAM.getRegisterNo(), displayMode, "", "register_no", "textbox", "", "")%></td>			 				 				 	
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "DEALER_CODE_NAME") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displayPopUpTagScriptAction(SEARCH_XCMR_DATAM.getDealerCode(),displayMode,"5","dealer_code","30","dealer_desc","textbox","","50","...","button_text","LoadDealer",ORIGDisplayFormatUtil.displayHTML(dealer),"Dealer") %></td>
								 </tr>			 
								 <tr>
								  	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "MAIN_CUSTOMER_TYPE") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction_ORIG(customer_typeVect,SEARCH_XCMR_DATAM.getCustomerType(),"customer_type",displayMode,"") %></td>
									<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "CHANNEL") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction_ORIG(channelVect,SEARCH_XCMR_DATAM.getChannel(),"channel",displayMode,"") %></td>
								 </tr>
								 <tr>			 	
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "PRIORITY") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction(priorityVect,SEARCH_XCMR_DATAM.getPriority(),"priority",displayMode,"") %></td>		 				 				 	
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "SORT_BY") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction(sort_byVect,SEARCH_XCMR_DATAM.getSortBy(),"sort_by",displayMode,"") %></td>			 				 	
								 </tr>
								 <tr>
					  			    <td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "REQUEST_ID") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(ORIGDisplayFormatUtil.displayHTML(SEARCH_XCMR_DATAM.getRequestID()), displayMode, "", "request_id", "textbox", "", "")%></td>		 				 				 	
...
<table id="resultTable" width="100%" height="100%">
	<tr>
		<td colspan="2"><iframe src="orig/xcmr/xcmr_override_search_result_screen.jsp" width="100%" height="100%" name="MainAppFrame" allowTransparency="true" style="background-color: #FFFFFF" frameborder="0"></iframe></td>
	</tr>
</table>
...
function searchApp(){
	if(validate()){
		...
		appFormName.action.value="SearchXCMR";
