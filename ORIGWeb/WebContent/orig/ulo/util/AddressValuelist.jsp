...
<%
	...
	logger.debug("#####AT PAGE>>"+SearchForm.getAtPage());	
	logger.debug("#####ITEM PAGE>>"+SearchForm.getItemsPerPage());
	String buttonFunction = request.getParameter("SEARCH_FUCNTION");
%>
	...
	<div class="col-xs-8">
		<div class="btnwrap" style="float: right">
			...
			<div style="float: left; padding: 5px 0;">
				<%=HtmlUtil.dropdown("itemsPerPage", "",FormatUtil.getString(itemsPerPage),SystemConstant.getConstant("FIELD_ID_ITEMS_PER_PAGE"), "", HtmlUtil.EDIT,HtmlUtil.elementTagId("itemsPerPage"), request) %>
			</div>
		</div>
	</div>
</div>
...
function clickPageList(atPage){
	...
	ADDRESS_SEARCH_BTNActionJS();
}
function itemsPerPageActionJS(){
	ADDRESS_SEARCH_BTNActionJS();
}
</script>
