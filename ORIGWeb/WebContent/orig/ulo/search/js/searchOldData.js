...
function getSearchOldDataPage() {
	return $('[name=searchOldDataPage]').val();
}
	...
	try{	
		...
		var checker = countFilledInput(formElement);
			...
			if (validateElementInputChars('[name=TH_FIRST_NAME],[name=TH_LAST_NAME],[name=COMPANY]')) {
				Pace.block();
				$('#action').val(getSearchOldDataPage());
				$('#handleForm').val('N');
				$('#appFormName').submit();
			} else {
...
function loadOldAppActionJS(applicationGroupId){
	try{
		Pace.block();
		$('[name=APPLICATION_GROUP_ID]').val(applicationGroupId);
		$('#action').val('LoadOldApplicationData');
		$('#handleForm').val('N');
		$('#appFormName').submit();
	}catch(exception){
		errorException(exception);
