...
<%@ page import="com.eaf.orig.shared.model.SearchCMRDataM" %>
...
<%
	SearchCMRDataM SEARCH_CMR_DATAM = (SearchCMRDataM) request.getSession().getAttribute("SEARCH_CMR_DATAM");
	if(SEARCH_CMR_DATAM == null){
		SEARCH_CMR_DATAM = new SearchCMRDataM();
	}
	System.out.println("SEARCH_CMR_DATAM ==> "+SEARCH_CMR_DATAM);
	...
	String dealer = cacheUtil.getORIGCacheDisplayNameFormDB(SEARCH_CMR_DATAM.getDealerCode(), OrigConstant.CacheName.CACHE_NAME_DEALER);
					...
					<table cellspacing=0 cellpadding=0 width="100%" align=center border=0>	     		
                        ...
                        <tr class="sidebar10"> <td align="center">
							<table width="100%" align=center cellpadding="0" cellspacing="1" border="0">
								<tr>
			 						<td class="textColS" width="22%"><%=MessageResourceUtil.getTextDescription(request, "CITIZEN_ID") %>:</td>
					 				<td class="inputCol" width="23%"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_CMR_DATAM.getCitizenID(), displayMode, "", "citizen_id", "textbox", "", "")%></td>
					 				<td class="textColS" width="20%"><%=MessageResourceUtil.getTextDescription(request, "APPLICATION_NO") %> :</td>
					 				<td class="inputCol" width="35%"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_CMR_DATAM.getApplicationNo(), displayMode, "", "application_no", "textbox", "", "")%></td>
					 			</tr>
					 			<tr>
								 	<td class="textColS" ><%=MessageResourceUtil.getTextDescription(request, "THAI_FNAME") %> :</td>
								 	<td class="inputCol" ><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_CMR_DATAM.getCustomerName(), displayMode, "", "company_name", "textbox", "", "")%></td>
								 	<td class="textColS" ><%=MessageResourceUtil.getTextDescription(request, "THAI_LNAME") %> :</td>
								 	<td class="inputCol" ><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_CMR_DATAM.getCustomerLName(), displayMode, "", "lastname", "textbox", "", "")%></td>			 				 	
								 </tr>
								 <tr>
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "VEHICLE_REGIS_NO") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displayInputTagScriptAction(SEARCH_CMR_DATAM.getRegisterNo(), displayMode, "", "register_no", "textbox", "", "")%></td>			 				 				 	
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "DEALER_CODE_NAME") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displayPopUpTagScriptAction(SEARCH_CMR_DATAM.getDealerCode(),displayMode,"5","dealer_code","30","dealer_desc","textbox","","50","...","button_text","LoadDealer",ORIGDisplayFormatUtil.displayHTML(dealer),"Dealer") %></td>
								 </tr>			 
								 <tr>
								  	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "MAIN_CUSTOMER_TYPE") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction_ORIG(customer_typeVect,SEARCH_CMR_DATAM.getCustomerType(),"customer_type",displayMode,"") %></td>
									<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "CHANNEL") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction_ORIG(channelVect,SEARCH_CMR_DATAM.getChannel(),"channel",displayMode,"") %></td>
								 </tr>
								 <tr>			 	
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "PRIORITY") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction(priorityVect,SEARCH_CMR_DATAM.getPriority(),"priority",displayMode,"") %></td>		 				 				 	
								 	<td class="textColS"><%=MessageResourceUtil.getTextDescription(request, "SORT_BY") %> :</td>
								 	<td class="inputCol"><%=ORIGDisplayFormatUtil.displaySelectTagScriptAction(sort_byVect,SEARCH_CMR_DATAM.getSortBy(),"sort_by",displayMode,"") %></td>			 				 	
								 </tr>
							</table>
						</td></tr>
					</table>
...
<TABLE id="resultTable" cellSpacing=0 cellPadding=0 width="100%" height="100%" border="0" bgcolor="#FFFFFF">
	<TR><TD>
		<iframe src="orig/cmr/cmr_search_result_screen.jsp" width="100%" height="100%" name="MainAppFrame" allowTransparency="true" style="background-color: #FFFFFF" frameborder="0"></iframe></td>
	</tr>
</table>
...
function validate(){
	...
	if( (citizen_id=="") && (application_no=="") && (company_name=="") && (lastname=="") && (register_no=="") && (dealer_code=="") && (customer_type=="") && (channel=="") && (priority=="") ){
...
function searchApp(){
	if(validate()){
		...
		appFormName.action.value="SearchCMR";
