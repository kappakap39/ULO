/* NumberFormat 1.5.0
 * v1.5.0 - 20-December-2002 * v1.0.3 - 23-March-2002 * v1.0.2 - 13-March-2002 * v1.0.1 - 20-July-2001 * v1.0.0 - 13-April-2000
 * http://www.mredkj.com * NumberFormat -The constructor * num - The number to be formatted. *  Also refer to setNumber
 */
...
/* setNumber - Sets the number * num - The number to be formatted
...
/* setNegativeFormat - How to format a negative number. * format - The format. Use one of the following constants.
 * LEFT_DASH   example: -1000 * RIGHT_DASH  example: 1000- * PARENTHESIS example: (1000)
 * v1.5.0 - new
 */
function setNegativeFormatNF(format){
	this.negativeFormat = format;
}
/* setNegativeRed - Format the number red if it's negative. * isRed - true, to format the number red if negative, black if positive;
 ...
 *	LEFT_OUTSIDE  example: $-1.00 *	LEFT_INSIDE   example: -$1.00 *	RIGHT_INSIDE  example: 1.00$- *	RIGHT_OUTSIDE example: 1.00-$
 * v1.5.0 - new
 */
...
function toFormattedNF(){
	...
	if (nStr.indexOf(this.PERIOD) == -1){
		splitString[0] = nStr;	splitString[1] = '';
	}else{
...
function justNumberNF(val){
  ...
  newVal = tempVal;	
	...
	// v1.0.3
	else if (!isFinite(newVal)){	newVal = 0;  }  
  ...
  // v1.5.0
  if (isPercentage){  	newVal = newVal / 100;  }	
	return newVal;
}