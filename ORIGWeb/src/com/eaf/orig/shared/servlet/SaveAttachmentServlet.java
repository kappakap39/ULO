 ...
 * @author Avalant
 * 
 * Type: SaveAttachmentServlet
 */
public class SaveAttachmentServlet extends HttpServlet implements Servlet {
    Logger logger = Logger.getLogger(SaveAttachmentServlet.class);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        logger.debug("<<<<<<< Start SaveAttachementServlet >>>>>>>");
        ...
        for (int index = 1; index <= maxDoc; index++) {
            ...
            try {
//                ORIGDAOFactory.getOrigAttachmentHistoryMDAO().saveUpdateModelOrigAttachmentHistoryM(attachHistoryDataM);
            	
            	PLORIGEJBService.getORIGDAOUtilLocal().saveUpdateModelOrigAttachmentHistoryM(attachHistoryDataM);
            	
            } catch (Exception e) {
                logger.debug(" Save update OrigAttachementHistoryError", e);
            }
...
//            ORIGDAOFactory.getOrigAttachmentHistoryMDAO().deleteNotInKeyTableORIG_ATTACHMENT_HISTORY( vAttach,appForm.getAppRecordID());
        ...
        //del
        String redirectUrl = request.getContextPath() + "/orig/appform/app_addimage.jsp" + queryString;
