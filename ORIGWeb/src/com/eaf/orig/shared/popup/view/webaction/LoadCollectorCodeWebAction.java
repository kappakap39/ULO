 ...
 * @author weeraya
 *
 * Type: LoadCollectorCodeWebAction
 */
public class LoadCollectorCodeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadCollectorCodeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        logger.debug("collector_code = "+collector_code);
        ...
        try {
			...
			sql.append("SELECT COTCDE, THDESC, ENDESC ");
			sql.append("FROM HPTBHP23 ");
            
			if(!ORIGUtility.isEmptyString(collector_code)){
			    sql.append("WHERE COTCDE = ? ");
			    valueListM.setString(++index,collector_code);
			    isFrist = false;
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, collector_code);
            }
