 ...
 * @author weeraya
 *
 * Type: LoadLicenseTypeWebAction
 */
public class LoadLicenseTypeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadLicenseTypeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("SELECT LICENSE_TYPE, TH_DESC, EN_DESC ");
			sql.append("FROM MS_LICENSE_TYPE ");
            
			if(!ORIGUtility.isEmptyString(car_license_type)){
			    sql.append("WHERE LICENSE_TYPE = ? ");
			    valueListM.setString(++index,car_license_type);
			    isFrist = false;
			...
			valueListM.setSearchAction("LoadLicenseType");
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, car_license_type);
            }
