 ...
 */
public class LoadLoanTypeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadLoanTypeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        logger.debug("loan_type = "+loan_type);
        ...
        try {
			...
			sql.append("SELECT LOANTYP, THDESC, ENDESC ");
			sql.append("FROM HPTBHP38 ");
            
			if(!ORIGUtility.isEmptyString(loan_type)){
			    sql.append("WHERE LOANTYP = ? ");
			    valueListM.setString(++index,loan_type);
			    isFrist = false;
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, loan_type);
            }
