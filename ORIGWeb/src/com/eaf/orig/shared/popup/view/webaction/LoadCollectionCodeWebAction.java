 ...
 * @author weeraya
 *
 * Type: LoadCollectionCodeWebAction
 */
public class LoadCollectionCodeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadCollectionCodeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        logger.debug("collection_code = "+collection_code);
        ...
        try {
			...
			sql.append("SELECT COLCDE, THDESC, ENDESC ");
			sql.append("FROM HPTBHP22 ");
            
			if(!ORIGUtility.isEmptyString(collection_code)){
			    sql.append("WHERE COLCDE = ? ");
			    valueListM.setString(++index,collection_code);
			    isFrist = false;
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, collection_code);
            }
