 ...
 * @author weeraya
 *
 * Type: LoadZipCodeWebAction
 */
public class LoadZipCodeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadZipCodeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        logger.debug(">>>>>>>>>>>>>>>>>>>>>>>>>> Start load zipcode <<<<<<<<<<<<<<<<<<<<<<<<<<<");
        ...
        try {
			...
			sql.append("SELECT ZIPCODE, ZIPCODE_ID ");
			sql.append("FROM MS_ZIPCODE ");
			
			if(!ORIGUtility.isEmptyString(code)){
			    sql.append("WHERE UPPER(ZIPCODE_ID) like ? ");
			    valueListM.setString(++index,"%"+code.toUpperCase()+"%");
			    isFirst = false;
			}else if(!ORIGUtility.isEmptyString(zipcode)){
