 ...
 */
public class LoadRepOfficeCodeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadRepOfficeCodeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("SELECT OFFCDE,ENNAM ");
			sql.append("FROM HPTBHP00 ");
            
			if(!ORIGUtility.isEmptyString(office_code)){
			    sql.append("WHERE UPPER(OFFCDE) = UPPER(?) ");
			    valueListM.setString(++index,office_code);
			    isFrist = false;
			...
			valueListM.setSearchAction("LoadRepOfficeCode");
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, office_code);
            }
