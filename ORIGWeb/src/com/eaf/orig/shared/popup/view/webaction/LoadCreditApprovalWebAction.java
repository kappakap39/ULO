 ...
 */
public class LoadCreditApprovalWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadCreditApprovalWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        logger.debug("credir_approval = "+credir_approval);
        ...
        try {
			...
			sql.append("WHERE DET.PRMTYP = HED.PRMTYP AND  HED.PRMTYP = 'CREDITAPRV' ");
			
			if(!ORIGUtility.isEmptyString(credir_approval)){
			    sql.append("AND DET.PRMCDE = ? ");
			    valueListM.setString(++index,credir_approval);
			...
			valueListM.setSearchAction("LoadCreditApproval");
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, credir_approval);
            }
