 ...
 * @author weeraya
 *
 * Type: LoadExternalCheckerWebAction
 */
public class LoadExternalCheckerWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadExternalCheckerWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("SELECT EXTCHK, THDESC, ENDESC ");
			sql.append("FROM HPTBHP40 ");
            
			if(!ORIGUtility.isEmptyString(external)){
			    sql.append("WHERE EXTCHK = ? ");
			    valueListM.setString(++index,external);
			    isFrist = false;
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, external);
            }
