 ...
 */
public class LoadLPMStatusWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadLPMStatusWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("select ACCOUNT_STATUS_CODE ,TH_ACCOUNT_DESCRIPTION,EN_ACCOUNT_DESCRIPTION ");
			sql.append("from NCB_ACCOUNT_STATUS ");
            
			if(!ORIGUtility.isEmptyString(lpm_status)){
			    sql.append("WHERE  UPPER(ACCOUNT_STATUS_CODE) = ? ");
			    valueListM.setString(++index,lpm_status.toUpperCase());
			    isFrist = false;
			}else if(!ORIGUtility.isEmptyString(code)){
			    sql.append("WHERE UPPER(TH_ACCOUNT_DESCRIPTION) like ? ");
			...
			valueListM.setSearchAction("LoadLPMStatus");
