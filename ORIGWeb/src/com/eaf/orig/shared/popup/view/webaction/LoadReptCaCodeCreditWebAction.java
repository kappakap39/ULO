 ...
 */
public class LoadReptCaCodeCreditWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadReptCaCodeCreditWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			if(!ORIGUtility.isEmptyString(tca_code_cr)){
			    sql.append("WHERE UPPER(PRMCDE) = UPPER(?) ");
			    valueListM.setString(++index,tca_code_cr);
			    isFrist = false;
			}else if(!ORIGUtility.isEmptyString(code)){
			    sql.append("WHERE UPPER(PRMCDE) like ? ");
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, tca_code_cr);
            }
