 ...
 * @author weeraya
 *
 * Type: LoadInternalCheckerWebAction
 */
public class LoadInternalCheckerWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadInternalCheckerWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("SELECT INTERNAL_CHECKER, TH_DESC, ACTIVE_STATUS ");
			sql.append("FROM MS_INTERNAL_CHECKER ");
            
			if(!ORIGUtility.isEmptyString(internal)){
			    sql.append("WHERE INTERNAL_CHECKER = ? ");
			    valueListM.setString(++index,internal);
			    isFrist = false;
			...
			sql.append("ORDER BY INTERNAL_CHECKER ");
			
			valueListM.setTextboxCode(textboxCode);
			valueListM.setTextboxDesc(textboxDesc);
			valueListM.setSearchAction("LoadInternalChecker");
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, internal);
            }
