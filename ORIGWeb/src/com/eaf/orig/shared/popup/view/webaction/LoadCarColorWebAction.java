 ...
 * @author weeraya
 *
 * Type: LoadCarColorWebAction
 */
public class LoadCarColorWebAction extends WebActionHelper implements WebAction {
    Logger logger = Logger.getLogger(LoadCarColorWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("SELECT COLOR, TH_DESC, EN_DESC ");
			sql.append("FROM MS_CAR_COLOR ");
			
			if(!ORIGUtility.isEmptyString(car_color)){
			    sql.append("WHERE COLOR = ? ");
			    valueListM.setString(++index,car_color);
			...
			valueListM.setSearchAction("LoadCarColor");
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, car_color);
            }
