 ...
 * @author weeraya
 *
 * Type: LoadInsuranceTypeWebAction
 */
public class LoadInsuranceTypeWebAction extends WebActionHelper implements WebAction {
    private static Logger logger = Logger.getLogger(LoadInsuranceTypeWebAction.class);
    ...
    public boolean preModelRequest() {
        ...
        try {
			...
			sql.append("SELECT INSTYP, THDESC, ENDESC ");
			sql.append("FROM HPTBIS02 ");
            
			if(!ORIGUtility.isEmptyString(ins_type)){
			    sql.append("WHERE INSTYP = ? ");
			    valueListM.setString(++index,ins_type);
			    isFrist = false;
            ...
            if(!ORIGUtility.isEmptyString(code)){
            	...
            	//getRequest().getSession().setAttribute(ValueListWebAction.VALUE_LIST_SEARCH_CODE, ins_type);
            }
