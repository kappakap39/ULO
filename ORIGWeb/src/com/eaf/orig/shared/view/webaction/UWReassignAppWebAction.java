 ...
 * @author weeraya
 *
 * Type: UWReassignAppWebAction
 */
public class UWReassignAppWebAction extends WebActionHelper implements WebAction {
	Logger logger = Logger.getLogger(UWReassignAppWebAction.class);
    /* (non-Javadoc)
...
//			String userName =userM.getUserName();
	    	String[] reassignChk = getRequest().getParameterValues("reassignChk");
	        ...
	        if (reassignChk != null) {
	        	String data;
	        	String[] datas;
	            for (int i = 0; i < reassignChk.length; i++) {
	                logger.debug(">>> reassignChk : " + reassignChk[i]);
	                data = reassignChk[i];
	                datas = data.split(",");
	                logger.debug(">>> datas.length : " + datas.length);
	                logger.debug(">>> datas.length : " + datas[datas.length-1]);
	                logger.debug(">>> appRecordId : " + datas[0]);
	                logger.debug(">>> appStatus : " + datas[1]);
	                logger.debug(">>> jobState : " + datas[2]);
	                logger.debug(">>> providerUrlWF : " + providerUrlWF);
	                logger.debug(">>> jndiWF : " + jndiWF);
	                manager.uwReAssignApplication(datas[0], datas[1], datas[2], userM, providerUrlWF, jndiWF);
	            }
	        }
    	}catch(Exception e){
