...
import com.eaf.orig.shared.model.SearchXUWDataM;
 ...
 *
 * Type: SearchXUWWebAction
 */
public class SearchXUWWebAction extends WebActionHelper implements WebAction {
	Logger logger = Logger.getLogger(SearchXUWWebAction.class);
	...
	public boolean preModelRequest() {
	    logger.debug("Search XUW Webaction");
		ORIGUtility utility = new ORIGUtility();
		SearchXUWDataM searchXUWDataM = (SearchXUWDataM)getRequest().getSession().getAttribute("SEARCH_XUW_DATAM");
		...
		if(searchXUWDataM == null){			
	     ...
	     //   logger.debug("dealer_fax = "+dealer_fax);
	        ...
	        searchXUWDataM = new SearchXUWDataM();
	        
	        searchXUWDataM.setCitizenID(citizen_id);
            searchXUWDataM.setCustomerName(company_name);
            searchXUWDataM.setCustomerLName(lastname);
            searchXUWDataM.setApplicationNo(application_no);
            searchXUWDataM.setRegisterNo(register_no);
            searchXUWDataM.setChannel(channel);
            searchXUWDataM.setDealerCode(dealer_code);
            searchXUWDataM.setCustomerType(customer_type);
            searchXUWDataM.setSortBy(sort_by);
            searchXUWDataM.setPriority(priority);
            searchXUWDataM.setRequestID(request_id);
            
            getRequest().getSession().setAttribute("SEARCH_UW_DATAM", searchXUWDataM);
            
		}else{
			 citizen_id = searchXUWDataM.getCitizenID();
			 company_name = searchXUWDataM.getCustomerName();
			 lastname = searchXUWDataM.getCustomerLName();
			 application_no= searchXUWDataM.getApplicationNo();
			 register_no = searchXUWDataM.getRegisterNo();
			 channel = searchXUWDataM.getChannel();
			 dealer_code = searchXUWDataM.getDealerCode();
			 customer_type = searchXUWDataM.getCustomerType();
			 sort_by = searchXUWDataM.getSortBy();
			 priority = searchXUWDataM.getPriority();
			 request_id = searchXUWDataM.getRequestID();			
		}
...
//            sql.append(" AND AIR.APPLICATION_RECORD_ID(+) = A.APPLICATION_RECORD_ID ");
            ...
            if("Override".equals(searchType)){ //Application Status new /jobstate
			    ...
			    valueListM.setString(++index,OrigConstant.JobState.XUW_NEW_STATE);
			    //sql.append(" AND XUW_POLICY_EXCEPTION='Y' ");
            ...
            valueListM.setReturnToAction("page=XUW_SEARCH_SCREEN");
