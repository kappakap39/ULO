 ...
 * Type: AppScoreAction
 */
public class AppScoreAction implements Action {

	Logger logger = Logger.getLogger(AppScoreAction.class);
	...
	public EventResponse perform(Event ev) {
		...
		AppScoreEvent event = (AppScoreEvent)ev;
		...
		try{
		    switch(eventType){
				case AppScoreEvent.APP_SCORE_SAVE:
						logger.debug("now i'm in case APP_SCORE_SAVE");
						manager.saveAppScore((ScoreM)object);
				    break;
				
				default:break;						
			}
		}catch(Exception e){
		    logger.error("AppScoreAction.perform error >>",e);
