 ...
 * Type: RunParamAction
 */
public class PolicyRulesAction implements Action {

	Logger logger = Logger.getLogger(PolicyRulesAction.class);
	...
	public EventResponse perform(Event ev) {
		...
		PolicyRulesEvent event = (PolicyRulesEvent)ev;
		...
		try{
		    switch(eventType){
				case PolicyRulesEvent.POLICY_RULES_SELECT:
					logger.debug("now i'm in case POLICY_RULES_SELECT");
					String policyCode = ((PolicyRulesDataM)object).getPolicyCode();
					String policyType = ((PolicyRulesDataM)object).getPolicyType();
					object = manager.selectPolicyRules(policyCode, policyType);
				    break;
				case PolicyRulesEvent.POLICY_RULES_UPDATE:
					logger.debug("now i'm in case POLICY_RULES_UPDATE");					
					manager.updatePolicyRules((PolicyRulesDataM)object);
				    break;		
				case PolicyRulesEvent.POLICY_RULES_DELETE:
					logger.debug("now i'm in case POLICY_RULES_DELETE");					
					manager.deletePolicyRules((Vector)object);
