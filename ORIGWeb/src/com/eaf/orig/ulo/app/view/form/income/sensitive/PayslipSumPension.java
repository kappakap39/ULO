...
public class PayslipSumPension  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(payslipList)) {
					for(IncomeCategoryDataM incomeTypeM: payslipList) {
						...
						payslipM.setSpacialPension(null);
					}
				}
			}
		}
		...
		if(!Util.empty(payslipList) && incomeOrig != null) {
			for(IncomeCategoryDataM incomeTypeM: payslipList) {
				...
				if(payslipM.getId() != null) {
					...
					if(payslipOrig != null &&  !CompareObject.compare(payslipM.getSpacialPension(), payslipOrig.getSpacialPension(),null)) {
						if(payslipM.getNoMonth() != 0) {
							formError.clearElement(getFieldName());
						} else {
							formError.clearElement("KBANK_PENSION2");
						}
					}
				} else {
					if(payslipM.getNoMonth() != 0) {
