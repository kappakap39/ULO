...
public class FixedAcctBalance  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(fixedAcctList)) {
					for(IncomeCategoryDataM incomeObj: fixedAcctList) {
						...
						othFixedAcctM.setAccountBalance(null);
					}
				}
			}
		}
		...
		if(!Util.empty(fixedAcctList)){
			for(IncomeCategoryDataM incomeObj: fixedAcctList) {
				...
				if(othFixedAcctM.getId() != null) {
					...
					if(fixedAcctOrig != null &&  !CompareObject.compare(othFixedAcctM.getAccountBalance(),fixedAcctOrig.getAccountBalance(),null)){
