...
public class DVFormDisplayObjectPrivilege extends FormDisplayObjectHelper{
	private static transient Logger logger = Logger.getLogger(DVFormDisplayObjectPrivilege.class);
	@Override
	...
	public String getObjectFormMode(String objectId, String objectType,Object objectElement) {
		...
		if(!Util.empty(personalInfo)){
			...
			if(!Util.empty(verificationResult)){
				...
				String requiredFlag = verificationResult.getRequiredVerPrivilegeFlag();
				if(!Util.empty(requiredFlag) && MConstant.FLAG.NO.equals(requiredFlag)) {
					displayMode=HtmlUtil.VIEW;
				} else if(null!=resultCode && resultCode.equals(SystemConstant.getConstant("VALIDATION_STATUS_WAIVED"))){
					displayMode=HtmlUtil.VIEW;
				}
			}
		}
