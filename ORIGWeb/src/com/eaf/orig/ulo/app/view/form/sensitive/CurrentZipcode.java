...
public class CurrentZipcode extends ElementHelper implements ElementInf{
	private static transient Logger logger = Logger.getLogger(CurrentZipcode.class);
	...
	public String getElement(HttpServletRequest request,Object objectElement) {		
		...
		String prefixAddress = CompareSensitiveFieldUtil.getPrefixAddress(personalInfoM,ADDRESS_TYPE_CURRENT);
		...
		.append(HtmlUtil.getFieldLabel(request, "ZIPCODE_"+ADDRESS_TYPE_CURRENT, "col-sm-4 col-md-5 control-label"))
		/*.append(HtmlUtil.numberBox(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData),null, "","#####","","", true, "5", displayMode,
				tagId, "col-sm-8 col-md-7",request))*/
		
		.append(HtmlUtil.popupList(CompareSensitiveFieldUtil.getElementName(compareData), CompareSensitiveFieldUtil.getElementSuffix(compareData), "", "", "50", displayMode, tagId, "col-sm-4 col-md-5 control-label", request))
		.append(HtmlUtil.hidden(prefixHiddenFiled+"_REKEY_ADDRESS_TYPE", ADDRESS_TYPE_CURRENT))
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			if(!Util.empty(address)) {
				...
				compareData.setValue(FormatUtil.display(address.getZipcode()));
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		if(!Util.empty(address)) {
			...
			address.setZipcode(request.getParameter(paramName));*/
			String prefixAddress = CompareSensitiveFieldUtil.getPrefixAddress(personalInfoM,ADDRESS_TYPE_CURRENT);
	...
	public HashMap<String,Object> validateElement(HttpServletRequest request, Object objectElement){
		...
		if(!Util.empty(personalInfoM)) {
			address = personalInfoM.getAddress(ADDRESS_TYPE_CURRENT);
		}
		if(Util.empty(address) || Util.empty(address.getZipcode())) {
			formError.error(compareData.getFieldName(),PREFIX_ERROR+LabelUtil.getText(request,"ZIPCODE_"+ADDRESS_TYPE_CURRENT));
		}*/
