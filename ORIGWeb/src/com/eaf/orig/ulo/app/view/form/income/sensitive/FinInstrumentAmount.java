...
public class FinInstrumentAmount  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(finInstrumentList)) {
					for(IncomeCategoryDataM incomeObj: finInstrumentList) {
						...
						finInstrumentM.setCurrentBalance(null);
					}
				}
			}
		}
...
//		PersonalInfoDataM personalInfoOrig = applicationGroup.getPersonalInfo(PERSONAL_TYPE);
		...
		if(!Util.empty(finInstrumentList)){
			for(IncomeCategoryDataM incomeObj: finInstrumentList) {
				FinancialInstrumentDataM finInstrumentM = (FinancialInstrumentDataM) incomeObj;
				if(finInstrumentM.getId() != null) {
					FinancialInstrumentDataM finInstrumentOrig = (FinancialInstrumentDataM)incomeOrig.getIncomeCategoryById(finInstrumentM.getId());
					if(finInstrumentOrig != null &&  !CompareObject.compare(finInstrumentOrig.getCurrentBalance(),finInstrumentM.getCurrentBalance(),null)){
						formError.clearElement(getFieldName()+"_"+finInstrumentM.getSeq());
					}
				} else {
					formError.clearElement(getFieldName()+"_"+finInstrumentM.getSeq());
				}
