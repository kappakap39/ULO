...
public class PaymentRatio extends ElementHelper implements ElementInf{
	private static transient Logger logger = Logger.getLogger(PaymentRatio.class);
	private static final String PRODUCT_K_PERSONAL_LOAN = SystemConstant.getConstant("PRODUCT_K_PERSONAL_LOAN");
	...
	public String getElement(HttpServletRequest request,Object objectElement) {
		...
		String bussClassId = applicationDataM.getBusinessClassId();
		String paymentMethod = paymentMethodDataM.getPaymentMethod();
		if(paymentMethod != null && paymentMethod.equals(PAYMENT_METHOD_DEPOSIT_ACCOUNT) && 
				PRODUCT_CODE_KEC.equals(product) && MConstant.FLAG_Y.equals(applicationGroup.foundLowIncome())) {
		...
		HTML.append("<div class='col-sm-12'>")
			...
			.append(HtmlUtil.dropdown(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData), "PAYMENT_RATIO", "","", FIELD_ID_PAYMENT_RATIO, 
					bussClassId, displayMode,tagId, "col-sm-8 col-md-7", request))
			.append("</div></div>")
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(allPaymentMethods)) {
			for(PaymentMethodDataM paymentMethod : allPaymentMethods) {
				...
				if(!PRODUCT_K_PERSONAL_LOAN.equals(paymentMethod.getProductType())) {
					...
					if(!Util.empty(linkedAppM)) {
						...
						compareData.setCurrentRefId(linkedAppM.getProduct());
