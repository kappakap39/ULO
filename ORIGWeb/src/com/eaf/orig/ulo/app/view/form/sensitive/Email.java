...
public class Email extends ElementHelper implements ElementInf {
...
//		String addressRefId = CompareSensitiveFieldUtil.getAddressRefId(personalInfoM,ADDRESS_TYPE_CURRENT);
		...
		.append(HtmlUtil.getFieldLabel(request, "EMAIL", "col-sm-4 col-md-5 control-label"))
		.append(HtmlUtil.textBoxEmail(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData), null, "", "", displayMode, tagId, "col-sm-8 col-md-7", request))
		.append("</div></div>")
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			compareData.setValue(personalInfoM.getEmailPrimary());
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		if(!Util.empty(personalInfoM)) {
			...
			personalInfoM.setEmailPrimary(Email);
		}
		return null;
	}
