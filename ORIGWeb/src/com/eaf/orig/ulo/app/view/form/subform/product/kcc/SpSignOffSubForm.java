...
public class SpSignOffSubForm extends ORIGSubForm{
	...
	public void setProperties(HttpServletRequest request, Object appForm) {
		...
		ArrayList<ApplicationDataM> applications = applicationGroup.filterApplicationLifeCycle(MConstant.Product.CREDIT_CARD);
		logger.debug("SpSignOffSubForm...");
		String  spSignOffDate = request.getParameter("SP_SIGN_OFF_DATE_CC");	
		String  spSignOffBy = request.getParameter("SP_SIGN_OFF_AUTHORIZED_BY_CC");	
	...
	public HashMap validateForm(HttpServletRequest request, Object appForm) {
		...
		formError.mandatoryDate(subformId, "SP_SIGN_OFF_DATE", "SP_SIGN_OFF_DATE_CC", spSignOffDate, request);
		formError.mandatory(subformId,"SP_SIGN_OFF_AUTHORIZED_BY","SP_SIGN_OFF_AUTHORIZED_BY_CC",spSignOffBy,request);
	...
	public void displayValueForm(HttpServletRequest request, Object objectForm){
		...
		if(!Util.empty(applications)){
			for(ApplicationDataM applicationDataM : applications){
				...
				applicationDataM.setSpSignoffDate(formValue.getValue("SP_SIGN_OFF_DATE_CC","SP_SIGN_OFF_DATE_"+SUFFIX_ELEMENT_ID,applicationDataM.getSpSignoffDate()));
				applicationDataM.setSpSignoffAuthBy(formValue.getValue("SP_SIGN_OFF_AUTHORIZED_BY_CC","SP_SIGN_OFF_AUTHORIZED_BY_"+SUFFIX_ELEMENT_ID,applicationDataM.getSpSignoffAuthBy()));
			}
		}
	}
