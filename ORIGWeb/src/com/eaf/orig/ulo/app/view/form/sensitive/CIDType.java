...
public class CIDType extends ElementHelper implements ElementInf{
	private static transient Logger logger = Logger.getLogger(CIDType.class);
	...
	public String getElement(HttpServletRequest request,Object objectElement) {		
		...
		.append(HtmlUtil.dropdown(CompareSensitiveFieldUtil.getElementName(compareData), CompareSensitiveFieldUtil.getElementSuffix(compareData), "", "", "", FIELD_ID_CID_TYPE, "", displayMode, tagId, "col-sm-8 col-md-7", request))
		.append("</div></div>")
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			compareData.setValue(personalInfoM.getCidType());
	...
	public String processElement(HttpServletRequest request,Object objectElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			if(!Util.empty(idNoVal)){
				personalInfoM.setCidType(idNoVal);
			}
		}		
		return null;
	}	
