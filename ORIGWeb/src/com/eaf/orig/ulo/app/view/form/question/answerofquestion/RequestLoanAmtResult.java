...
public class RequestLoanAmtResult  extends FormHelper implements FormAction {
	private static transient Logger logger = Logger.getLogger(RequestLoanAmtResult.class);
			 ...
			 if(!Util.empty(vcCardDataMList)){
				 ...
				 while(iterator.hasNext()){
					 VcCardDataM vcCardData= iterator.next();
						if(Util.empty(vcCardData.getTEMP_CR_LMT_AMT()) && Util.empty(vcCardData.getCARD_TP())){
							iterator.remove();
						} 
					}
			 }
