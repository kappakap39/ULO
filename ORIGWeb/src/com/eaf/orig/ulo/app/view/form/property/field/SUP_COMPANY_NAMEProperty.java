...
public class SUP_COMPANY_NAMEProperty extends FieldPropertyHelper {
	private static transient Logger logger = Logger.getLogger(SUP_COMPANY_NAMEProperty.class);
...
//		PersonalInfoDataM personalInfo = ORIGForm.getObjectForm().getPersonalInfo(PERSONAL_TYPE_SUPPLEMENTARY);	
...
//		EntityForm.getObjectForm();
		PersonalInfoDataM personalInfo=personalApplicationInfo.getPersonalInfo();
		if(Util.empty(personalInfo)){
			personalInfo = new PersonalInfoDataM();
		}
		logger.debug("SUP_UNMANDATORY_COMPANY_NAME BY OCCUPATION >> "+personalInfo.getOccupation());
		ArrayList<String>  UNMANDATORY_COMPANY_NAME =  SystemConstant.getArrayListConstant("UNMANDATORY_COMPANY_NAME");
		if(!Util.empty(personalInfo.getOccupation()) && !UNMANDATORY_COMPANY_NAME.contains(personalInfo.getOccupation())){
