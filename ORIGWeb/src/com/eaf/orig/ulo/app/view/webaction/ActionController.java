...
public class ActionController extends WebActionHelper implements WebAction {
	...
	public boolean preModelRequest(){		
		...
		String ACTION_TYPE = storeAction.get("ACTION_TYPE");	
		String FORM_ROLE = storeAction.get("FORM_ROLE");	
		
		logger.debug("ROLE >> "+ROLE);
		logger.debug("ACTION_TYPE >> "+ACTION_TYPE);
		logger.debug("FORM_ROLE >> "+FORM_ROLE);
		
		if(MConstant.FLAG.YES.equals(FIRST_ACTION)){
			userM.setCurrentRole(ROLE);
			userM.setRoleMenu(ROLE);
			flowControl.setRole(ROLE);
			flowControl.setActionType(ACTION_TYPE);
			flowControl.setFormRole(FORM_ROLE);
		}
...
//		#rawi comment change to used cache
...
//		HashMap<String,String> hTable = QueryEngine.LoadModule("SELECT A.* FROM METAB_ACTION_PARAM A WHERE A.MENU_ID = ?",MENU_ID);
		HashMap<String, Object> hTable = CacheControl.get("MetabActionParam",MENU_ID);
		String ACTION_PARAM = SQLQueryEngine.display(hTable,"ACTION_PARAM"); 
		String ROLE_NAME = SQLQueryEngine.display(hTable,"ROLE_NAME"); 
		try{
		...
		storeAction.put("ROLE",ROLE_NAME);
		return storeAction;
	}
	@Override
