...
public class CompanyName extends ElementHelper implements ElementInf{
	...
	private static transient Logger logger = Logger.getLogger(CompanyName.class);
	@Override
...
//		String addressRefId = CompareSensitiveFieldUtil.getAddressRefId(personalInfoM,ADDRESS_TYPE_WORK);
		...
		.append(HtmlUtil.autoComplete(CompareSensitiveFieldUtil.getElementName(compareData), CompareSensitiveFieldUtil.getElementSuffix(compareData), "COMPANY_NAME_LISTBOX", "", "", "", "ALL_ALL_ALL"
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			if(!Util.empty(address)) {
				...
				compareData.setValue(address.getCompanyName());
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		ApplicationGroupDataM applicationGroup = (ApplicationGroupDataM)RekeyForm.getObjectForm();		
		CompareDataM compareData = ((CompareDataM)objectElement);
		AddressDataM workAddress = CompareSensitiveFieldUtil.getAddressByUniqueId(applicationGroup, compareData);
		if(!Util.empty(workAddress)) {
			String paramName = CompareSensitiveFieldUtil.getParameterName(compareData);
			logger.debug("paramName>>"+paramName);
			String companyName = request.getParameter(paramName);
	...
	public HashMap<String,Object> validateElement(HttpServletRequest request, Object objectElement){
		...
		AddressDataM workAddress = null;
		if(!Util.empty(personalInfoM)) {
			workAddress = personalInfoM.getAddress(ADDRESS_TYPE_WORK);
		}		
		if(Util.empty(workAddress) || Util.empty(workAddress.getCompanyName())) {
