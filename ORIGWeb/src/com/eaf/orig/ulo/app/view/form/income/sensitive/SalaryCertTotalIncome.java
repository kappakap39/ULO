...
public class SalaryCertTotalIncome  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(salaryCertList)) {
					for(IncomeCategoryDataM incomeObj: salaryCertList) {
						...
						salaryCertM.setTotalIncome(null);
					}
				}
			}
		}
		...
		if(!Util.empty(salaryCertList)) {
			for(IncomeCategoryDataM incomeObj: salaryCertList) {
				...
				if(salaryCertM.getId() != null) {
					...
					if(salaryCertOrig != null &&  !CompareObject.compare(salaryCertM.getTotalIncome(), salaryCertOrig.getTotalIncome(),null)) {
