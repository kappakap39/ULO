...
public class PayrollFileAmount  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(incomeList)) {
					for(IncomeCategoryDataM incomeTypeM: incomeList) {
						PayrollFileDataM payrollFileM = (PayrollFileDataM) incomeTypeM;
						if(!MConstant.FLAG.YES.equals(payrollFileM.getFromFileFlag())) {
							payrollFileM.setAmount(null);
						}
					}
				}
			}
...
//		PersonalInfoDataM personalInfoOrig = applicationGroup.getPersonalInfo(PERSONAL_TYPE);
		...
		if(!Util.empty(incomeList)&& incomeOrig != null) {
			for(IncomeCategoryDataM incomeTypeM: incomeList) {
				PayrollFileDataM payrollFileM = (PayrollFileDataM) incomeTypeM;
				if(!MConstant.FLAG.YES.equals(payrollFileM.getFromFileFlag())) {
					if(payrollFileM.getId() != null) {
						PayrollFileDataM payrollOrig = (PayrollFileDataM)incomeOrig.getIncomeCategoryById(payrollFileM.getId());
						if(payrollOrig != null &&  !CompareObject.compare(payrollFileM.getAmount(), payrollOrig.getAmount(),null)) {
							formError.clearElement(getFieldName()+"_"+payrollFileM.getSeq());
						}
					} else {
						formError.clearElement(getFieldName()+"_"+payrollFileM.getSeq());
					}
