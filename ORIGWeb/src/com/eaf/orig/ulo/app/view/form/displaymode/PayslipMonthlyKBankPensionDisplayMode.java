...
public class PayslipMonthlyKBankPensionDisplayMode extends FormDisplayModeHelper{
	private static transient Logger logger = Logger.getLogger(PayslipMonthlyKBankPensionDisplayMode.class);
	@Override
	public String displayMode(Object objectElement, HttpServletRequest request) {
		...
		if(!Util.empty(personalInfo)){
			idNo = personalInfo.getIdno();
		}
		logger.debug(">>idNo>>"+idNo);
		logger.debug(">>elementId>>"+elementId);
		DihDAO dihDAO = new DihDAOImpl();
		try {
			if(!Util.empty(idNo) && dihDAO.isKbankEmployee(idNo) && Util.empty(payslipM.getNoMonth())){
