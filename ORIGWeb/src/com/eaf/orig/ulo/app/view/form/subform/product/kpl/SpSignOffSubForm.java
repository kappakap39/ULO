...
public class SpSignOffSubForm  extends ORIGSubForm{
	...
	String subformId ="KPL_SP_SIGN_OFF_SUBFORM";
	@Override
	public void setProperties(HttpServletRequest request, Object appForm) {
		ApplicationGroupDataM applicationGroup = ((ApplicationGroupDataM)appForm);	
		ArrayList<ApplicationDataM> applications = applicationGroup.filterApplicationLifeCycle(MConstant.Product.K_PERSONAL_LOAN);
		
		String spSignOffDate = request.getParameter("SP_SIGN_OFF_DATE_KPL");
		String spSignOffBy = request.getParameter("SP_SIGN_OFF_AUTHORIZED_BY_KPL");
	...
	public HashMap validateForm(HttpServletRequest request, Object appForm) {
		ApplicationGroupDataM applicationGroup = (ApplicationGroupDataM)appForm;
		ApplicationDataM application = applicationGroup.filterApplicationProductLifeCycle(MConstant.Product.K_PERSONAL_LOAN);
		Date spSignOffDate = application.getSpSignoffDate();	
		String spSignOffBy = application.getSpSignoffAuthBy();
		FormErrorUtil formError = new FormErrorUtil();	
		formError.mandatoryDate(subformId, "SP_SIGN_OFF_DATE", "SP_SIGN_OFF_DATE_KPL", spSignOffDate, request);
		formError.mandatory(subformId,"SP_SIGN_OFF_AUTHORIZED_BY","SP_SIGN_OFF_AURTHORIZED_BY_KPL",spSignOffBy,request);
	...
	public void displayValueForm(HttpServletRequest request, Object objectForm){
		...
		if(!Util.empty(applications)){
			for(ApplicationDataM  applicationDataM : applications){
				...
				applicationDataM.setSpSignoffDate(formValue.getValue("SP_SIGN_OFF_DATE_KPL","SP_SIGN_OFF_DATE_"+SUFFIX_ELEMENT_ID,applicationDataM.getSpSignoffDate()));
				applicationDataM.setSpSignoffAuthBy(formValue.getValue("SP_SIGN_OFF_AURTHORIZED_BY_KPL","SP_SIGN_OFF_AUTHORIZED_BY_"+SUFFIX_ELEMENT_ID,applicationDataM.getSpSignoffAuthBy()));
			}
		}
	}
