...
public class LastNameTH extends ElementHelper implements ElementInf {
	private static transient Logger logger = Logger.getLogger(LastNameTH.class);
...
//		String personalRefId = CompareSensitiveFieldUtil.getPersonalRefId(personalInfoM);	
		...
		if(CIDTYPE_PASSPORT.equals(personalInfoM.getCidType())){
			HTML.append(HtmlUtil.textBoxEN(CompareSensitiveFieldUtil.getElementName(compareData), CompareSensitiveFieldUtil.getElementSuffix(compareData),"","","50",displayMode,
					tagId, "col-sm-8 col-md-7",request));
		}else{
			HTML.append(HtmlUtil.textBoxTH(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData),"","","50",displayMode,
					tagId, "col-sm-8 col-md-7",request));
		}
		HTML.append("</div></div>");
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			compareData.setValue(personalInfoM.getThLastName());
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		if(!Util.empty(personalInfoM)) {
			...
			String lastName = request.getParameter(paramName);
			personalInfoM.setThLastName(lastName);
		}
		return null;
	}	
	@Override
	public HashMap<String,Object> validateElement(HttpServletRequest request, Object objectElement){
		...
		if(!Util.empty(personalInfoM)) {
			if(CIDTYPE_PASSPORT.equals(personalInfoM.getCidType())){
				if(PersonalInfoUtil.validateRegExpByFieldName(personalInfoM,TH_LAST_NAME)){
