...
public class BUNDLE_SME_G_NEW_PAY_REQProperty extends FieldPropertyHelper {
	private static transient Logger logger = Logger.getLogger(BUNDLE_SME_G_NEW_PAY_REQProperty.class);
	@Override
	...
	public String validateFlag(HttpServletRequest request, Object objectForm) {
		logger.debug("BUNDLE_SME_G_NEW_PAY_REQProperty.validateFlag");
		...
		if(!Util.empty(bundleSMEM)) {
			...
			if(!Util.empty(BORROWING_TYPE)) { 
				if(LOAN_TYPE_COBORROWER_JOINT.equals(BORROWING_TYPE)){
					...
					if((Util.empty(bundleSMEM.getVerifiedIncome()) && Util.empty(bundleSMEM.getgTotNewPayReq()))
							|| (!Util.empty(bundleSMEM.getVerifiedIncome()) && !Util.empty(bundleSMEM.getgTotNewPayReq()))){
						return ValidateFormInf.VALIDATE_YES;
					}
				}			
			}
	...
	public HashMap<String,Object> validateForm(HttpServletRequest request, Object objectForm) {	
		logger.debug("BUNDLE_SME_G_NEW_PAY_REQProperty.validateForm");
		...
		if(!Util.empty(bundleM)) {    
			if(LOAN_TYPE_COBORROWER_JOINT.equals(bundleM.getBorrowingType())){
				if(Util.empty(bundleM.getgTotNewPayReq())) {
					formError.error("G_TOTAL_NEW_PAY_REQ",PREFIX_ERROR+LabelUtil.getText(request,"G_TOTAL_NEW_PAY_REQ"));
				}
			}
				...
				if(Util.empty(bundleM.getgDscrReq()) && Util.empty(bundleM.getgTotExistPayment())
					...
					formError.error("G_TOTAL_NEW_PAY_REQ",ERROR_NOT_FILL_VERIFIED_INCOME_G_FIELD);
				}
					...
					formError.error("G_TOTAL_NEW_PAY_REQ",ERROR_ALL_IN_VERIFIED_INCOME_G_FIELD);
				}
				else if(!Util.empty(bundleM.getgDscrReq()) || !Util.empty(bundleM.getgTotExistPayment())){
					if(Util.empty(bundleM.getgTotNewPayReq())) {
						formError.error("G_TOTAL_NEW_PAY_REQ",ERROR_G_FIELD);
					}
				}
			}
		}
