...
public class CurrentAddressStatus  extends ElementHelper implements ElementInf{
	private static transient Logger logger = Logger.getLogger(CurrentAddressStatus.class);
...
//		String addressRefId = CompareSensitiveFieldUtil.getAddressRefId(personalInfoM,ADDRESS_TYPE_CURRENT);
		...
		.append(HtmlUtil.getFieldLabel(request,"ADRSTS_"+ADDRESS_TYPE_CURRENT, "col-sm-4 col-md-5 control-label"))
		.append(HtmlUtil.dropdown(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData), "ADRSTS_"+ADDRESS_TYPE_CURRENT, "",
				 "",FIELD_ID_ADRSTS,"ALL_ALL_ALL",displayMode,tagId,"col-sm-8 col-md-7", request))
		.append("</div></div>")
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			if(!Util.empty(address)) {
				...
				compareData.setValue(address.getAdrsts());
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		AddressDataM address = CompareSensitiveFieldUtil.getAddressByUniqueId(applicationGroup, compareData);
		if(!Util.empty(address)) {
			String addressSts = request.getParameter(CompareSensitiveFieldUtil.getParameterName(compareData));
			address.setAdrsts(addressSts);
		}
		return null;
	}	
	@Override
	public HashMap<String,Object> validateElement(HttpServletRequest request, Object objectElement){
		...
		if(Util.empty(address) || Util.empty(address.getAdrsts())) {
