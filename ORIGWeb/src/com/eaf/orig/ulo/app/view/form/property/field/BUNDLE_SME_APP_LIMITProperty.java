...
public class BUNDLE_SME_APP_LIMITProperty extends FieldPropertyHelper {
	private static transient Logger logger = Logger.getLogger(BUNDLE_SME_APP_LIMITProperty.class);
	@Override
	...
	public HashMap<String,Object> validateForm(HttpServletRequest request, Object objectForm) {	
		logger.debug("BUNDLE_SME_APP_LIMITProperty.validateForm");
		String PREFIX_ERROR = MessageErrorUtil.getText(request,"PREFIX_ERROR");
		BundleSMEDataM bundleM = (BundleSMEDataM)objectForm;
		FormErrorUtil formError = new FormErrorUtil();
		String LOAN_TYPE_SINGLE = SystemConstant.getConstant("LOAN_TYPE_SINGLE");
		if(!Util.empty(bundleM) && LOAN_TYPE_SINGLE.equals(bundleM.getBorrowingType())){
			if(Util.empty(bundleM.getApprovalLimit())) {
				formError.error("APPROVAL_LIMIT",PREFIX_ERROR+LabelUtil.getText(request,"APPROVAL_LIMIT"));
			}
		}
