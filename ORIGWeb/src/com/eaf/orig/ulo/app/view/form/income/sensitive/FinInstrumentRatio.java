...
public class FinInstrumentRatio  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(finInstrumentList)) {
					for(IncomeCategoryDataM incomeObj: finInstrumentList) {
						...
						finInstrumentM.setHoldingRatio(null);
					}
				}
			}
		}
		...
		if(!Util.empty(finInstrumentList)){
			for(IncomeCategoryDataM incomeObj: finInstrumentList) {
				...
				if(finInstrumentM.getId() != null) {
					...
					if(finInstrumentOrig != null &&  !CompareObject.compare(finInstrumentOrig.getHoldingRatio(),finInstrumentM.getHoldingRatio(),null)){
