...
public class ReferenceOfficePhoneNo extends ElementHelper implements ElementInf {
	private static transient Logger logger = Logger.getLogger(ReferenceOfficePhoneNo.class);
	@Override
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(referencePersonList)) {
			for(ReferencePersonDataM referencePerson : referencePersonList) {
				...
				compareData.setValue(referencePerson.getOfficePhone());
