...
public class TaweesapAum  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
		...
		if(Util.empty(applicationGroup.getReprocessFlag())){
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(incomeList)) {
					TaweesapDataM taweesapM = (TaweesapDataM)incomeList.get(0);
					if(taweesapM != null) {
						taweesapM.setAum(null);
					}
				}
			}
		}
...
//		PersonalInfoDataM personalInfoOrig = applicationGroup.getPersonalInfo(PERSONAL_TYPE);
		...
		if(!Util.empty(incomeList) && incomeOrig != null) {
			TaweesapDataM taweesapM = (TaweesapDataM)incomeList.get(0);
			if(taweesapM != null && taweesapM.getId() != null) {
				TaweesapDataM taweesapOrig = (TaweesapDataM)incomeOrig.getIncomeCategoryById(taweesapM.getId());
				if(taweesapOrig != null &&  !CompareObject.compare(taweesapM.getAum(), taweesapOrig.getAum(),null)) {
