...
public class CallForCashAccountNo extends ElementHelper implements ElementInf{
	...
	String SUFFIX_CALL_FOR_CASH = PRODUCT_K_EXPRESS_CASH+CompareDataM.MARKER+CALL_FOR_CASH;	
	...
	public String getElement(HttpServletRequest request,Object objectElement) {		
		...
		String elementId = FormatUtil.getCashTransferElementId(personalInfoM,PRODUCT_K_EXPRESS_CASH,CALL_FOR_CASH);
...
//		ApplicationDataM application = applicationGroup.filterApplicationProductLifeCycle(PRODUCT_K_EXPRESS_CASH);
		if(!Util.empty(applicationInfo)) {
			...
			HTML.append("<div class='col-sm-12'>")
				...
				.append(HtmlUtil.getFieldLabel(request, "CALL_FOR_CASH_ACCT_NO", "col-sm-4 col-md-5 control-label"))
				...
				.append(HtmlUtil.hidden("PRODUCT_NAME_"+CompareSensitiveFieldUtil.getParameterName(compareData),SUFFIX_CALL_FOR_CASH))
...
/*			.append(HtmlUtil.textBoxAccountNo("CALL_FOR_CASH_ACCOUNT_NO", SUFFIX_CALL_FOR_CASH,"","",displayMode, 
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(application)) {
				...
				if(null != callForCash){
					compareData.setValue(callForCash.getTransferAccount());
				}
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		if(!Util.empty(application)){
			...
			if(!Util.empty(callForCash)) {
				...
				String transferAcct = request.getParameter(paramName);
				transferAcct = FormatUtil.removeDash(transferAcct);
				logger.debug("transferAcct : "+transferAcct);
				callForCash.setTransferAccount(transferAcct);
			}
		}
		return null;
	}	
