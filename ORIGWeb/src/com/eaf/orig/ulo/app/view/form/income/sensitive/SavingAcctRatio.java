...
public class SavingAcctRatio  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(savAcctList)) {
					for(IncomeCategoryDataM incomeObj: savAcctList) {
						...
						othSavingAcctM.setHoldingRatio(null);
					}
				}
			}
		}
...
//		PersonalInfoDataM personalInfoOrig = applicationGroup.getPersonalInfo(PERSONAL_TYPE);
		...
		if(!Util.empty(savAcctList)) {
			for(IncomeCategoryDataM incomeObj: savAcctList) {
				SavingAccountDataM othSavingAcctM = (SavingAccountDataM) incomeObj;
				if(othSavingAcctM.getId() != null) {
					SavingAccountDataM savAcctOrig = (SavingAccountDataM)incomeOrig.getIncomeCategoryById(othSavingAcctM.getId());
					if(savAcctOrig != null &&  !CompareObject.compare(othSavingAcctM.getHoldingRatio(),savAcctOrig.getHoldingRatio(),null)){
						if(MConstant.FLAG.YES.equals(getClearFlag())) {
							othSavingAcctM.setHoldingRatio(null);
						}
						formError.clearElement(getFieldName()+"_"+othSavingAcctM.getSeq());
					}
				} else {
					formError.clearElement(getFieldName()+"_"+othSavingAcctM.getSeq());
				}
