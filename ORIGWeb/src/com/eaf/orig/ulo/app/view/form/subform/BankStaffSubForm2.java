...
public class BankStaffSubForm2 extends ORIGSubForm{
	private static transient Logger logger = Logger.getLogger(BankStaffSubForm2.class);
...
//		ArrayList<String> products = applicationGroup.getProducts(applicationGroup.getMaxLifeCycle());
		...
		String APPROVE_BY = request.getParameter("APPROVE_BY");
		...
		applicationGroup.setEmployeeName(APPROVE_BY);
		applicationGroup.setEmployeeNo(APPROVE_ID);
		applicationGroup.setTransactNo(TRANSACT);
...
//		formError.mandatory(subformId,"APPROVE_BY",saleInfoRefSale.get,request);
//		formError.mandatory(subformId,"APPROVE_ID",saleInfoRefSale.get,request);
//		formError.mandatory(subformId,"TRANSACT",saleInfoRefSale.get,request);
	...
	public String renderSubformFlag(HttpServletRequest request,	Object objectForm) {
		...
		String BANK_STAFF_SUBFORM_2 = SystemConstant.getConstant("BANK_STAFF_SUBFORM_2");	
		
		String checkProductType = render.determineBankStaff(templateId);
		if(BANK_STAFF_SUBFORM_2.equals(checkProductType)){
