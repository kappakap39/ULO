...
public class PayslipSumBonus  extends CompareIncomeHelper implements CompareIncomeInf{
	private static transient Logger logger = Logger.getLogger(PayslipSumBonus.class);
	@Override
		...
		if(!Util.empty(payslipList) && incomeOrig != null) {
			for(IncomeCategoryDataM incomeTypeM: payslipList) {
				...
				if(payslipM.getId() != null) {
					...
					if(payslipOrig != null &&  !CompareObject.compare(payslipM.getBonus(), payslipOrig.getBonus(),null)) {
						logger.info("Not same '"+payslipOrig.getBonus()+"'");
						if(payslipM.getNoMonth() != 0) {
							formError.clearElement(getFieldName());
						} else {
							formError.clearElement("BONUS");
						}
					}
				} else {
					if(payslipM.getNoMonth() != 0) {
