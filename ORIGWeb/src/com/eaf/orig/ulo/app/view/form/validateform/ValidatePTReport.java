...
public class ValidatePTReport extends ValidateFormHelper implements ValidateFormInf {
	private static transient Logger logger = Logger.getLogger(ValidatePTReport.class);
	...
	public HashMap<String,Object> validateForm(HttpServletRequest request, Object objectForm) {
		...
		if(Util.empty(DATE_FROM_CALENDAR)){
			formError.mandatoryElement("PT_REPORT_DATE_FROM", "PT_REPORT_DATE_FROM", DATE_FROM_CALENDAR, request);
		}
		if(Util.empty(DATE_TO_CALENDAR)){
			formError.mandatoryElement("PT_REPORT_DATE_TO", "PT_REPORT_DATE_TO", DATE_TO_CALENDAR, request);
		}
		if(Util.empty(COMPLETE_REPORT_RADIO)){
			formError.mandatoryElement("PT_REPORT_COMPLETE", "PT_REPORT_COMPLETE", COMPLETE_REPORT_RADIO, request);
		}
//		if(Util.empty(PROJECT_NO_BOX)){
//			formError.mandatoryElement("PT_REPORT_PROJECT_NO", "PT_REPORT_PROJECT_NO", PROJECT_NO_BOX, request);
//		}
