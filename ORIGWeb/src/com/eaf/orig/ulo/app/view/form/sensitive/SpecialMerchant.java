...
public class SpecialMerchant extends ElementHelper implements ElementInf{
	private static transient Logger logger = Logger.getLogger(SpecialMerchant.class);
	String REKEY_FORM = SystemConstant.getConstant("REKEY_FORM");
	String SPECIAL_MERCHANT_TYPE = SystemConstant.getConstant("FIELD_ID_SPECIAL_MERCHANT_TYPE");
	@Override
	public String getElement(HttpServletRequest request,Object objectElement) {		
		...
		.append(HtmlUtil.dropdown(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData), compareData.getFieldNameType(), "", "", 
				SPECIAL_MERCHANT_TYPE,"ALL_ALL_ALL", displayMode,tagId, "col-sm-8 col-md-7", request))
		.append("</div></div>")
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			compareData.setValue(personalInfoM.getSpecialMerchantType());
	...
	public String processElement(HttpServletRequest request,Object objectElement) {
		...
		if(!Util.empty(personalInfoM)) {
			...
			String specialMerchantType = request.getParameter(paramName);
			personalInfoM.setSpecialMerchantType(specialMerchantType);
		}		
		return null;
	}	
	@Override
