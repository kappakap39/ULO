...
public class _DecisionDV2 extends ProcessActionHelper {
	private static transient Logger logger = Logger.getLogger(_DecisionDV2.class);
	...
	private String DECISION_SERVICE_APPPROVE = SystemConstant.getConstant("DECISION_SERVICE_APPPROVE");
	private String DECISION_SERVICE_REFERRED = SystemConstant.getConstant("DECISION_SERVICE_REFERRED");
	private String FINAL_APP_DECISION_APPROVE = SystemConstant.getConstant("FINAL_APP_DECISION_APPROVE");
	private String FINAL_APP_DECISION_REFER = SystemConstant.getConstant("FINAL_APP_DECISION_REFER");
	private String DECISION_SERVICE_FOLLOW_UP = SystemConstant.getConstant("DECISION_SERVICE_FOLLOW_UP");
	...
	public Object processAction() {
		...
		try{
							...
							||!ApplicationGroupDataM.DecisionLog.REQUIRE_VERIFY_INCOME.equals(incomeFlowAction)))
			{
					...
					boolean skipCallDecision = ((isKPL && curDecisionAP) || isLowIncome);
					if(skipCallDecision)
					{
					...
					{
						DecisionServiceResponse decisionResponse = processDecisionDV2(decisionRequest);
			...
			}else if(!Util.empty(overSLAFlag))
			{
				...
				DecisionServiceResponse   decisionResponse = processDecisionDC(decisionRequest);
		...
		logger.debug("decisionApplication LowIncomeFlagLowIncomeFlagLowIncomeFlag: "+ decisionApplication.getLowIncomeFlag());
		return decisionApplication;
	}
	...
	public DecisionServiceResponse  processDecisionDV2(DecisionServiceRequestDataM decsionRequest){		
		DecisionServiceResponse responseData = new DecisionServiceResponse();
		try {
			decsionRequest.setDecisionPoint(DecisionServiceUtil.DECISION_POINT.DECISION_POINT_DV2);
	...
	public DecisionServiceResponse processDecisionDC(DecisionServiceRequestDataM decsionRequest){
		DecisionServiceResponse  decisionResponse = new DecisionServiceResponse();
		try {
			decsionRequest.setDecisionPoint(DecisionServiceUtil.DECISION_POINT.DECISION_POINT_DC);
