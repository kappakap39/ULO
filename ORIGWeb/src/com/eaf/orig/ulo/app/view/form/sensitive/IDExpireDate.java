...
public class IDExpireDate extends ElementHelper implements ElementInf{
	private static transient Logger logger = Logger.getLogger(IDExpireDate.class);
	...
	public <T>Object getObjectElement(HttpServletRequest request,Object objectForm,FieldElement fieldElement){
		...
		if(!Util.empty(personalInfoM)&&CIDTYPE_IDCARD.equals(personalInfoM.getCidType())) {
			...
			compareData.setValue(FormatUtil.display(personalInfoM.getCidExpDate(),HtmlUtil.EN));
	...
	public String processElement(HttpServletRequest request,Object objectElement){
		...
		if(!Util.empty(personalInfoM)) {
			...
			String idnoexp = request.getParameter(paramName);
			if(!Util.empty(idnoexp)){
				personalInfoM.setCidExpDate(FormatUtil.toDate(idnoexp, HtmlUtil.EN));
			}
		}		
		return null;
	}
	...
	public HashMap<String,Object> validateElement(HttpServletRequest request, Object objectElement){
		...
		if(!Util.empty(personalInfoM) && !Util.empty(personalInfoM.getCidExpDate())){
			if(DateValidateUtil.validateDate(personalInfoM.getCidExpDate(), DateValidateUtil.FORMAT_DATE_VALIDATE,DateValidateUtil.EN)
					|| DateValidateUtil.validateDate(personalInfoM.getCidExpDate(), DateValidateUtil.LIMIT_YEAR_DATE_VALIDATE,DateValidateUtil.EN)){
				formError.error(MessageErrorUtil.getText(request, "ERROR_DATE_FORMAT"));
			}
			if(DateValidateUtil.validateDate(personalInfoM.getCidExpDate(), DateValidateUtil.EXPIRE_DATE_VALIDATE,DateValidateUtil.EN)){
				formError.error(MessageErrorUtil.getText(request, "ERROR_IDNO_EXPIRE_DATE"));
			}
