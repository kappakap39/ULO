...
public class KviPercent  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(kviList)) {
					for(IncomeCategoryDataM incomeTypeM: kviList) {
						...
						kviM.setPercentChequeReturn(null);
					}
				}
			}
		}
		...
		if(!Util.empty(kviList) && incomeOrig != null) {
			for(IncomeCategoryDataM incomeTypeM: kviList) {
				...
				if(kviM.getId() != null) {
					...
					if(kviOrig != null &&  !CompareObject.compare(kviM.getPercentChequeReturn(), kviOrig.getPercentChequeReturn(),null)) {
