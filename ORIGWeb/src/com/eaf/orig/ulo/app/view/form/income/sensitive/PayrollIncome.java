...
public class PayrollIncome  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
		...
		if(Util.empty(applicationGroup.getReprocessFlag())){
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(payrollList)) {
					for(IncomeCategoryDataM incomeTypeM: payrollList) {
						PayrollDataM payrollM = (PayrollDataM)incomeTypeM;
						payrollM.setIncome(null);
					}
				}
			}
		}
...
//		PersonalInfoDataM personalInfoOrig = applicationGroup.getPersonalInfo(PERSONAL_TYPE);
		...
		if(!Util.empty(payrollList) && incomeOrig != null) {
			for(IncomeCategoryDataM incomeTypeM: payrollList) {
				PayrollDataM payrollM = (PayrollDataM)incomeTypeM;
				if(payrollM.getId() != null) {
					PayrollDataM payrollOrig = (PayrollDataM)incomeOrig.getIncomeCategoryById(payrollM.getId());
					if(payrollOrig != null &&  !CompareObject.compare(payrollM.getIncome(), payrollOrig.getIncome(),null)) {
