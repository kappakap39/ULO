...
public class CashTransferType extends ElementHelper implements ElementInf {
	private final String FIELD_ID_CASH_TRANSFER_TYPE =SystemConstant.getConstant("FIELD_ID_CASH_TRANSFER_TYPE");
	private final String PRODUCT_K_EXPRESS_CASH = SystemConstant.getConstant("PRODUCT_K_EXPRESS_CASH");
	...
	private static transient Logger logger = Logger.getLogger(CashTransferType.class);
	@Override
...
//		ApplicationDataM application = applicationGroup.filterApplicationProductLifeCycle(PRODUCT_K_EXPRESS_CASH);
		if(!Util.empty(applicationInfo)) {
			...
			.append(HtmlUtil.dropdown(CompareSensitiveFieldUtil.getElementName(compareData),CompareSensitiveFieldUtil.getElementSuffix(compareData), compareData.getFieldNameType(), "", "",FIELD_ID_CASH_TRANSFER_TYPE,
...
//		String []cashTransferType = ListBoxControl.searchListBox(FIELD_ID_CASH_TRANSFER_TYPE,"CHOICE_NO",request);
		...
		if(!Util.empty(application)) {
			...
			logger.debug("getObjectElement.cashTransfer : "+cashTransfer);
			PersonalInfoDataM personalInfo = applicationGroup.getPersonalInfoByRelation(application.getApplicationRecordId());
				...
				if(null != cashTransfer){
					compareData.setValue(cashTransfer.getCashTransferType());
				}
...
//		String []cashTransferType = ListBoxControl.searchListBox(FIELD_ID_CASH_TRANSFER_TYPE,"CHOICE_NO",request);
		String []cashTransferType = ApplicationUtil.getCashTranferTypes();
		logger.debug("cashTransferType : "+cashTransferType);
		ApplicationDataM application = applicationGroup.filterApplicationProductLifeCycle(PRODUCT_K_EXPRESS_CASH);
		if(!Util.empty(application)) {
			CashTransferDataM cashTransfer = application.getCashTransfer(cashTransferType);	
			...
			if(!Util.empty(cashTransfer)) {
				...
				String transferType = request.getParameter(paramName);
				logger.debug("transferType : "+transferType);
				if(!Util.empty(transferType)){
					cashTransfer.setCashTransferType(transferType);
				}
			}
		}
		return null;
...
//		String []cashTransferType = ListBoxControl.searchListBox(FIELD_ID_CASH_TRANSFER_TYPE,"CHOICE_NO",request);	
		...
		if(null != application){
			CashTransferDataM cashTransfer = application.getCashTransfer(cashTransferType);		
			logger.debug("validateElement.cashTransfer : "+cashTransfer);
			String transferType = request.getParameter("RK_CASH_TRANSFER_TYPE_"+SUFFIX_CASH_TRANSFER);
			if(!Util.empty(cashTransfer) && Util.empty(transferType)) {
				formError.error("RK_CASH_TRANSFER_TYPE_"+SUFFIX_CASH_TRANSFER,PREFIX_ERROR+LabelUtil.getText(request,compareData.getFieldNameType()));
			}
		}*/
