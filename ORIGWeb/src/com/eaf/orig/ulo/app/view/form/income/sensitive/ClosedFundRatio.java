...
public class ClosedFundRatio  extends CompareIncomeHelper implements CompareIncomeInf{
	@Override
			...
			if(MConstant.FLAG.YES.equals(getClearFlag())) {
				...
				if(!Util.empty(closedEndList)) {
					for(IncomeCategoryDataM incomeObj: closedEndList) {
						...
						othClosedFundM.setHoldingRatio(null);
					}
				}
			}
		}
		...
		if(!Util.empty(closedEndList)){
			for(IncomeCategoryDataM incomeObj: closedEndList) {
				...
				if(othClosedFundM.getId() != null) {
					...
					if(closedEndOrig != null &&  !CompareObject.compare(othClosedFundM.getHoldingRatio(),closedEndOrig.getHoldingRatio(),null)){
