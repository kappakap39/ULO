...
public class ProcessCardLinkAction {
	private static transient Logger logger = Logger.getLogger(ProcessCardLinkAction.class);	
	...
	ArrayList<CardLinkRow> cardLinkRows = new ArrayList<ProcessCardLinkAction.CardLinkRow>();
		...
		if(SystemConstant.lookup("APPLICATION_TYPE_NORMAL",applicationType)&&PRODUCT_CRADIT_CARD.equals(productId)){
			if(cardLinkItem == 0){
				...
				if(APPLICATION_CARD_TYPE_BORROWER.equals(applicationCardType)){
					...
					String mainUniqueId = application.getMaincardRecordId();
					int mainSeq = cardLinkRow.getSeq(mainUniqueId);
					cardLinkSeq = (SUP_CARDLINK_SEQ-1)+mainSeq;
				}
			}
		}else{
			if(cardLinkItem == 0){
...
//		System.out.println(refId.substring(7,9));
	...
	public void doAction(ApplicationGroupDataM applicationGroup){
		...
		if(Util.empty(cardLinkInfo.getCardLinkRefNo())){
			String cardlinkRefNo = ORIGServiceProxy.getGeneratorManager().generateCardLinkRefNo();
							...
							if(!Util.empty(borrowers)){
								for(ApplicationDataM borrower : borrowers){
									if(SystemConstant.lookup("PROCESS_CARDLINK_PRODUCT",productId)){
										String uniqueId = borrower.getApplicationRecordId();
		...
		if(!Util.empty(cardLinkRows)){
			for(CardLinkRow cardLinkRow : cardLinkRows){
				...
				if(PRODUCT_CRADIT_CARD.equals(cardLinkRow.getProductId())){
					cardLinkRow.setVlinkFlag(getVlinkFlag(applicationGroup));	
				}else{
					cardLinkRow.setVlinkFlag(CardLinkRow.Flag.NO_ACTION);
				}
	...
	public String getVlinkFlag(ApplicationGroupDataM applicationGroup){
		...
		//PROD Defect: Change to get channel from sale 20170614
		String saleChannel = saleInfo.getSaleChannel();
		logger.debug("saleChannel : "+saleChannel);
		if(SystemConstant.lookup("PROCESS_VLINK_APPLY_CHENNEL",saleChannel)
	...
	public class CardLinkRow implements Serializable,Cloneable{		
		...
		private ArrayList<CardLink> cardLinks = new ArrayList<ProcessCardLinkAction.CardLink>();
