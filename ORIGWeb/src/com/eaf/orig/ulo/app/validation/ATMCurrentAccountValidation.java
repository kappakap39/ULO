...
public class ATMCurrentAccountValidation extends AccountValidationHelper {
	private static transient Logger logger = Logger.getLogger(ATMCurrentAccountValidation.class);	
	public ATMCurrentAccountValidation(){
		super();
	}
	@Override
	public String validateField(HttpServletRequest request,	Object objectForm) {
		String ERR_CODE_ACCT_APPLICANT_ONLY_ATM_CURRENT = SystemConstant.getConstant("ERR_CODE_ACCT_APPLICANT_ONLY_ATM_CURRENT");
		String ERR_CODE_ACCT_SINGLE_ACCT_ATM_CURRENT = SystemConstant.getConstant("ERR_CODE_ACCT_SINGLE_ACCT_ATM_CURRENT");
		...
		if(Util.empty(cisNo)) {
			...
			setErrorCode(ERR_CODE_ACCT_APPLICANT_ONLY_ATM_CURRENT);
			}
		}else{
			...
			if(cisNo != null && cisNo.equals(personalInfo.getCisNo()) &&
					CIS_SRC_STM_CD != null && CIS_SRC_STM_CD.equals(AccountInformationDataM.AcctTableName.CURRENT.getCode())) {
			...
			if(AR_OWN_TP_CD != null && AR_OWN_TP_CD.equals(DIH_SINGLE_OWNER_CODE)) {
				...
				setErrorCode(ERR_CODE_ACCT_SINGLE_ACCT_ATM_CURRENT);
			}			
			if(!Util.empty(getAdditionalAccountNo())) {
				...
				if(cisNo != null && cisNo.equals(personalInfo.getCisNo()) &&
						CIS_SRC_STM_CD2 != null && CIS_SRC_STM_CD2.equals(AccountInformationDataM.AcctTableName.SAVING.getCode())) {
					logger.info("Acct valid");
				}else{
					setErrorCode(ERR_CODE_ACCT_APPLICANT_ONLY_ATM_CURRENT);
				}		
