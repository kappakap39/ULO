...
public class CashDay1AccountValidation extends AccountValidationHelper {

	private static transient Logger logger = Logger.getLogger(CashDay1AccountValidation.class);
	
	public CashDay1AccountValidation(){
		super();
	}

	@Override
	public String validateField(HttpServletRequest request,	Object objectForm) {
		String ERR_CODE_ACCT_APPLICANT_ONLY_CASH_TRANSFER = SystemConstant.getConstant("ERR_CODE_ACCT_APPLICANT_ONLY_CASH_TRANSFER");
		String ERR_CODE_ACCT_SINGLE_ACCT_CASH_TRANSFER = SystemConstant.getConstant("ERR_CODE_ACCT_SINGLE_ACCT_CASH_TRANSFER");
		String ERR_CODE_ACCT_SAVING_OR_CURRENT_CASH_TRANSFER = SystemConstant.getConstant("ERR_CODE_ACCT_SAVING_OR_CURRENT_CASH_TRANSFER");
		...
		if(Util.empty(CIS_INFO)) {
			if(Util.empty(getErrorCode())){
				setErrorCode(ERR_CODE_ACCT_APPLICANT_ONLY_CASH_TRANSFER);
			}
		} else {
			...
			if(ResponseData.SUCCESS.equals(dihCIS_SRC_STM_CD.getStatusCode())){
				...
				if(CIS_SRC_STM_CD != null &&
					...
					setErrorCode(ERR_CODE_ACCT_SAVING_OR_CURRENT_CASH_TRANSFER);
				}
				...
				if(ResponseData.SUCCESS.equals(dihAR_OWN_TP_CD.getStatusCode())){
					...
					if(AR_OWN_TP_CD != null && AR_OWN_TP_CD.equals(DIH_SINGLE_OWNER_CODE)) {
						...
						setErrorCode(ERR_CODE_ACCT_SINGLE_ACCT_CASH_TRANSFER);
					}
				}else{
