...
public class CCMainAccountValidation extends AccountValidationHelper {

	private static transient Logger logger = Logger.getLogger(CCMainAccountValidation.class);
	
	public CCMainAccountValidation(){
		super();
	}

	@Override
	public String validateField(HttpServletRequest request,	Object objectForm) {
		String ERR_CODE_ACCT_APPLICANT_ONLY_CC = SystemConstant.getConstant("ERR_CODE_ACCT_APPLICANT_ONLY_CC");
		String ERR_CODE_ACCT_SINGLE_JOINT_MAIN = SystemConstant.getConstant("ERR_CODE_ACCT_SINGLE_JOINT_MAIN");
		String ERR_CODE_ACCT_SAVING_OR_CURRENT_CC_MAIN = SystemConstant.getConstant("ERR_CODE_ACCT_SAVING_OR_CURRENT_CC_MAIN");
		...
		if(Util.empty(CIS_INFO)) {
			if(Util.empty(getErrorCode())){
				setErrorCode(ERR_CODE_ACCT_APPLICANT_ONLY_CC);
			}
		} else {
			...
			if(ResponseData.SUCCESS.equals(dihCIS_SRC_STM_CD.getStatusCode())){
				...
				if(CIS_SRC_STM_CD != null &&
					...
					logger.info("Acct Saving or current");
				} else {
					setErrorCode(ERR_CODE_ACCT_SAVING_OR_CURRENT_CC_MAIN);
				}
				...
				if(ResponseData.SUCCESS.equals(dihAR_OWN_TP_CD.getStatusCode())){
					...
					if(AR_OWN_TP_CD != null && AR_OWN_TP_CD.equals(DIH_SINGLE_OWNER_CODE)) {
						...
						setErrorCode(ERR_CODE_ACCT_SINGLE_JOINT_MAIN);
					}
				}else{
