...
import com.eaf.orig.model.It.UploadUserM;
...
public class DownloadUser implements DownloadControl{
	private static Logger logger = Logger.getLogger(DownloadUser.class);
	String IMPORT_USER_OUTPUT_NAME = SystemConfig.getProperty("IMPORT_USER_OUTPUT_NAME");
	...
	public ProcessResponse processDownload(HttpServletRequest request)throws Exception {
		...
		try{
			...
			if(OrigDownloadFileUtil.DOWNLOAD_PROCESS_NAME.DOWNLOAD_INVALID.equals(processName)){
				...
				ArrayList<UploadUserM> listUserM = dao.loadUserDataM();
				ArrayList<Map<String, Object[]>> dataList = new ArrayList<Map<String, Object[]>>();
				if(!Util.empty(listUserM)){
					for(UploadUserM data : listUserM){
						Map<String, Object[]> hData = new TreeMap<String, Object[]>();
						hData.put(OrigDownloadFileUtil.EXCEL_DATA_KEY,new Object[] {
							replaceNullWithSpace(data.getUserName()),
							replaceNullWithSpace(data.getEmpId()),
							replaceNullWithSpace(data.getNameTH()),
							replaceNullWithSpace(data.getSurnameTH()),
							replaceNullWithSpace(data.getNameEN()),
							replaceNullWithSpace(data.getSurnameEN()),
							replaceNullWithSpace(data.getDepartment()),
							replaceNullWithSpace(data.getOrganizePosition()),
							replaceNullWithSpace(data.getWorkPosition()),
							replaceNullWithSpace(data.getTelephone()),
							replaceNullWithSpace(data.getTelephoneEXT()),
							replaceNullWithSpace(data.getMobile()),
							replaceNullWithSpace(data.geteMail()),
							replaceNullWithSpace(data.getDLA()),
							replaceNullWithSpace(data.getJobLevel()),
							replaceNullWithSpace(data.getSkill()),
							replaceNullWithSpace(data.getProfile()),
							replaceNullWithSpace(data.getRole()),
							replaceNullWithSpace(data.getStatus())
						});
					dataList.add(hData);
		        	}
				}
				String fileName = OrigDownloadFileUtil.getGenerateFileName(IMPORT_USER_OUTPUT_NAME); 
				OrigDownloadFileUtil.createExceldRecordData(dataList,fileName,IMPORT_USER_TEMPLATE_FILE,DOWNLOAD_PATH);
				download.setFileName(fileName);
				download.setFilePath(DOWNLOAD_PATH);
				download.setMimeType("multipart/form-data");
				processResponse.setStatusCode(ServiceResponse.Status.SUCCESS);
