...
public class DownloadAttachmentSubForm implements DownloadControl{
	private static transient Logger logger = Logger.getLogger(DownloadAttachmentSubForm.class);	
	@Override
	public ProcessResponse processDownload(HttpServletRequest request) throws Exception {		
		...
		try {
			if (request.getParameter("ATTACHMENT_ID") != null) {
				...
				OrigAttachmentHistoryDAO dao = ModuleFactory.getAttachmentHistoryDAO();
				try{
					attachmentM = dao.loadAttachmentInfo(attachmentId);
				}catch(Exception e){
