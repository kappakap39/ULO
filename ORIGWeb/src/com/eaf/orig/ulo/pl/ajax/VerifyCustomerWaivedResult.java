...
public class VerifyCustomerWaivedResult implements AjaxDisplayGenerateInf{
	...
	public String getDisplayObject(HttpServletRequest request)throws PLOrigApplicationException {
		...
		if(HTMLRenderUtil.DISPLAY_MODE_EDIT.equals(displayMode)){
			...
			XrulesResponseDataM reponse = this.ExecuteILOGVerifyCustomer(appM, userM);	
			...
			if(null != reponse && PLXrulesConstant.ExecuteCode.EXE_SUCCESS.equals(reponse.getExecuteCode())){
				if(PLXrulesConstant.ResultCode.CODE_WAIVED.equals(reponse.getResultCode())){
					...
					if(OrigUtil.isEmptyString(xrulesVerM.getVerCusResultCode())){	
						...
						xrulesVerM.setVerCusResultCode(reponse.getResultCode());
						xrulesVerM.setVerCusResult(reponse.getResultDesc());
						...
						jObj.CreateJsonObject(verCusCodeField,PLXrulesConstant.ResultCode.CODE_WAIVED);
						jObj.CreateJsonObject(verCusResultField,PLXrulesConstant.ResultDesc.CODE_WAIVED_DESC);
					...
					if(PLXrulesConstant.ResultCode.CODE_WAIVED.equals(xrulesVerM.getVerCusResultCode())){
						...
						String verCusResultField = "result_"+PLXrulesConstant.ModuleService.VERIFY_CUSTOMER;
						String verCusCodeField = "code_"+PLXrulesConstant.ModuleService.VERIFY_CUSTOMER;					
						jObj.CreateJsonObject(verCusCodeField,"");
						jObj.CreateJsonObject(verCusResultField,"");
...
//			}
		...
		jObj.CreateJsonObject("td-vercus-final-staus", HTMLRenderUtil.displaySelectTagScriptAction_ORIG(v,xrulesVerM.getVerCusResultCode()
																	,"vercus-final-status",displayMode,""));	
		return jObj.returnJson();
	}
	
	public XrulesResponseDataM ExecuteILOGVerifyCustomer(PLApplicationDataM appM ,UserDetailM userM){		
...
//			if(OrigUtil.isEmptyString(xrulesVerM.getVerCusResultCode())){
				...
				XrulesRequestDataM requestM = tool.MapXrulesRequestDataM(appM, PLXrulesConstant.ModuleService.VERIFY_CUSTOMER, userM);
