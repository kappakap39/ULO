...
public class SearchReAllocateWebAction extends WebActionHelper	implements WebAction{
	private static transient Logger logger = Logger.getLogger(SearchReAllocateWebAction.class);
	...
	public boolean preModelRequest() {
		...
		try {
			...
			String ROLE =searchForm.getString("ROLE");
			String COMMA = "";
			if(!Util.empty(ROLE)){
				roleList.append(COMMA+ROLE.replace("_", "."));
				COMMA = ",";	
				setReferenceRole(ROLE,roleList,COMMA);			
			}else{
			...
			logger.debug(">>>>STAFF_NAME>>>"+searchForm.getString("STAFF_NAME"));
...
//        sql.append(" RE_ALOCATE.DUE_DATE,");
        sql.append(" DECODE(APP_GROUP.CLAIM_BY,'','"+MConstant.FLAG_N+"','"+MConstant.FLAG_Y+"') AS CLAIM_FLAG,");
        sql.append(" APP_GROUP.INSTANT_ID,");
        sql.append(" APP_GROUP.APPLICATION_GROUP_NO,");
        sql.append(" APP_GROUP.APPLICATION_STATUS,");
        sql.append(" TO_CHAR(APP_GROUP.UPDATE_DATE,'dd/mm/yyyy | HH24:MI',");
        sql.append(" 'NLS_CALENDAR=''THAI BUDDHA'' NLS_DATE_LANGUAGE=THAI') AS LAST_UPDATE ");                              
        ...
        sql.append(" JOIN LSW_USR_XREF U ON U.USER_ID = T.USER_ID");
...
//        sql.append(" INNER JOIN (SELECT * FROM TABLE(BPM_INBOX.GET_REALLOCATABLE_INSTANCE(?,?))) RE_ALOCATE ON RE_ALOCATE.BPD_INSTANCE_ID = APP_GROUP.INSTANT_ID");
        sql.append(" WHERE ( 1=1)");
//        sql.append(" AND APP_GROUP.JOB_STATE IS NOT NULL");
//			searchForm.setString(rolesList);
...
/*    	if(!searchForm.empty("APPLICATION_TYPE")){
    		sql.append(" AND   APP_GROUP.APPLICATION_TYPE= ? ");
    		searchForm.setString(searchForm.getString("APPLICATION_TYPE"));
    	}*/
		...
		if(owner != null && !owner.isEmpty()){//Search App by user_name
			...
			String assignableClause =  " AND UPPER(U.USER_NAME) = UPPER(?) ";
...
//					" JOIN LSW_USR_XREF U ON T.USER_ID = U.USER_ID " +
...
//					" AND T.STATUS = 12 " +
        ...
        sql.append(" APP_GROUP.INSTANT_ID,");
        sql.append(" APP_GROUP.APPLICATION_GROUP_NO,");
//        sql.append(" APP_GROUP.APPLICATION_DATE,");
        sql.append(" APP_GROUP.APPLICATION_STATUS,");
        sql.append(" APP_GROUP.UPDATE_DATE,");
        sql.append(" APP_GROUP.SLA_DUE_DATE,");
//        sql.append(" RE_ALOCATE.OWNER,");
//        sql.append(" RE_ALOCATE.TASK_ID,");
//        sql.append(" RE_ALOCATE.DUE_DATE");
        sql.append(" U.USER_NAME,");
        sql.append(" T.TASK_ID");
		sql.append(" ORDER BY APP_GROUP.PRIORITY , APP_GROUP.SLA_DUE_DATE");
		return sql.toString();
	}
	...
	private String subSQL() {
		...
		StringBuilder subSql = new StringBuilder();
			subSql.append(" PKA_SEARCH_INFO.GET_PRODUCT(M_RESULT.APPLICATION_GROUP_ID) AS PRODUCT_NAME ");
