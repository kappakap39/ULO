...
public class SearchReAssignWebAction extends WebActionHelper implements WebAction {
	private static transient Logger logger = Logger.getLogger(SearchReAssignWebAction.class);
...
//		searchForm.setString(searchForm.getString("STAFF_NAME"));
		...
		/*if (!searchForm.empty("PRODUCT")) {
			...
			String ORG_ID = CacheControl.getName(SystemConstant.getConstant("FIELD_ID_PRODUCT_TYPE"), searchForm.getString("PRODUCT"),
					"SYSTEM_ID1");
...
//		sql.append(" APP_GROUP.CREATE_DATE,");
//		sql.append(" APP_GROUP.JOB_STATE,");
		sql.append(" APP_GROUP.SLA_DUE_DATE,");
//		sql.append(" RE_ASSIGN.OWNER,");
//		sql.append(" RE_ASSIGN.TASK_ID,");
//		sql.append(" RE_ASSIGN.DUE_DATE");
		sql.append(" U.USER_NAME,");
		sql.append(" T.TASK_ID,");
		sql.append(" APP_GROUP.SLA_DUE_DATE");
		sql.append(" ORDER BY CASE WHEN OWNER IS NULL THEN 0 ELSE 1 END ASC, APP_GROUP.PRIORITY , APP_GROUP.SLA_DUE_DATE");
