...
public class LogonEngine {
	...
	public static final String ERROR_PLEASE_CONTRACT_ADMIN = "ERROR_PLEASE_CONTRACT_ADMIN";
	...
	public static void create(){
		...
		logonErrorCode.put(ERROR_PLEASE_CONTRACT_ADMIN,"1004");
	...
	public static void processLogon(HttpServletRequest request,String type){
		...
		if(null != logon){
			...
			try{
				ControlLogManager controlLog = ORIGServiceProxy.getControlLogManager();
				controlLog.saveLog(logon);
				controlLog.updateLogonFlag(logon.getUserName(),MConstant.FLAG.YES);
				WorkflowDAOFactory.getInboxDAO().updateTableUserInboxInfo(logon.getUserName(),MConstant.FLAG.YES);
			}catch(Exception e){
	...
	public static void processLogout(HttpSessionEvent request,String type){
		...
		LogonDataM logon = (LogonDataM)request.getSession().getAttribute("LogonData");
		if(null != logon){
			logon.setAction(MConstant.LOGOUT);	
			logon.setType(type);
			logon.setCreateDate(ApplicationDate.getTimestamp());
			try{
				ControlLogManager controlLog = ORIGServiceProxy.getControlLogManager();
				controlLog.saveLog(logon);
				controlLog.updateLogonFlag(logon.getUserName(),MConstant.FLAG.NO);
				WorkflowDAOFactory.getInboxDAO().updateTableUserInboxInfo(logon.getUserName(),MConstant.FLAG.NO);
			}catch(Exception e){
				logger.fatal("ERROR ",e);
			}
		}
	...
	public static String getErrorMsg(HttpServletRequest request){
		...
		try{
			...
			if("1014".equals(errorCode)){
				...
				errorMsg = MessageErrorUtil.getText(request,getKey(errorCode));
			}
			if(Util.empty(errorMsg)){
				errorMsg = "";
			}											
	...
	public static void init(HttpServletRequest request,HttpServletResponse response){
		logger.debug("init.origLogon...");
		try{
			String userName = (String)request.getSession().getAttribute("userName");
			logger.debug("userName >> "+userName);
			if(!Util.empty(userName)){
				ControlLogManager controlLog = ORIGServiceProxy.getControlLogManager();
				controlLog.updateLogonFlag(userName,MConstant.FLAG.NO);
			}	
