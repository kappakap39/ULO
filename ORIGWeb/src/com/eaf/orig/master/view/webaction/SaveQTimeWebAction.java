 ...
 *
 * Type: SaveQTimeWebAction
 */
public class SaveQTimeWebAction extends WebActionHelper implements WebAction {
	Logger log = Logger.getLogger(SaveQTimeWebAction.class);
	...
	public Event toEvent() {
		...
		qTimeEvent.setEventType(QTimeEvent.Q_TIME_SAVE);
		log.debug("QTimeEvent.Q_TIME_SAVE=" + QTimeEvent.Q_TIME_SAVE);
...
//		clear session for search field
		...
		QueueTimeOutM queueTimeOutM = (QueueTimeOutM)getRequest().getSession().getAttribute("ADD_Q_TIME_M");
		Vector qTimeLoanTypeMVect = new Vector();
		
		if(queueTimeOutM!= null){
			qTimeLoanTypeMVect = queueTimeOutM.getQTimeOutLoanTypeMVect();
		}
