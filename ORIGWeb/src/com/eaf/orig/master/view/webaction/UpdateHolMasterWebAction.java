 ...
 *
 * Type: UpdateHolMasterWebAction
 */
public class UpdateHolMasterWebAction extends WebActionHelper implements
		WebAction {
	Logger log = Logger.getLogger(UpdateHolMasterWebAction.class);
	...
	public Event toEvent() {
		...
		holMasterEvent.setEventType(HolMasterEvent.HOL_MASTER_UPDATE);
		
		log.debug("HolMasterEvent.HOL_MASTER_UPDATE=" + HolMasterEvent.HOL_MASTER_UPDATE);
	...
	public boolean preModelRequest() {
		...
		String holidayDate = getRequest().getParameter("holidayDate");
		String holidayDesc = getRequest().getParameter("holidayDesc");
		...
		String userName = userM.getUserName();
		 ...
		 log.debug("holidayDesc ="+holidayDesc);
		 log.debug("workingFlag ="+workingFlag);
		 		 
		 holidayM = new HolidayM(); 
		 
		 holidayM.setHolDate(ORIGUtility.parseThaiToEngDate(holidayDate));
		 holidayM.setHolDesc(holidayDesc);
		 ...
		 origMasterForm.setShwAddFrm("edit");
			...
			if (origMasterForm.getFormErrors()!=null && origMasterForm.getFormErrors().size() > 0){
				getRequest().getSession().setAttribute("EDIT_HOLIDAY_DATAM",holidayM);
				return false;
			}else {
				return true;
			}
    ...
    protected void doSuccess(EventResponse arg0) {
		...
		getRequest().getSession().removeAttribute("EDIT_HOLIDAY_DATAM");
