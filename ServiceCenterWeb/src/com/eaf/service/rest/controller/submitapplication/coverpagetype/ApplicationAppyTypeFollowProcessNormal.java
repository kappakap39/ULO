...
public class ApplicationAppyTypeFollowProcessNormal implements ApplicationAppyTypeProcess {
		...
		try{
			...
			if(!Util.empty(applicationGroupId)){
				...
				if(SystemConfig.containsGeneralParam("WIP_JOBSTATE_WAIT_FOLLOW_FU",jobState)){
					String[] decisionPoint={DECISION_SERVICE_POINT_BUREAU_DOC};
					processApplicationDecisionService(inquireDocSetResponse,applicationGroup,decisionPoint,userId);
					...
					String []decisionPoint={DECISION_SERVICE_POINT_DOC_COMPLETE,DECISION_SERVICE_POINT_INCOME};
					
					//Defect#2424, 2449 Add logic to skip DECISION_SERVICE_POINT_INCOME if follow SavingPlus
					if(isSavingPlus(applicationGroup) && JOBSTATE_FOLLOW_SAVING_PLUS.equals(jobState))
					{
						decisionPoint = new String[]{DECISION_SERVICE_POINT_DOC_COMPLETE};
					}
	...
	private void processApplicationDecisionService(InquireDocSetResponse inquireDocSetResponse,ApplicationGroupDataM applicationGroup,String[] decisionPoints,String userId)throws Exception{
		if(!Util.empty(decisionPoints)){
			for(String decisionPoint : decisionPoints){
				logger.debug("decisionPoint : "+decisionPoint);
				SubmitApplication submitApplication = new SubmitApplication();
					submitApplication.mapImageIndex(inquireDocSetResponse,applicationGroup);
					submitApplication.requestDecisionService(applicationGroup,decisionPoint,SystemConfig.getProperty("SYSTEM_FOLLOW_UP_USER_ID"));
			}
		}
		LookupServiceCenter.getServiceCenterManager().saveApplication(applicationGroup,userId,true);
	}
	
	public static boolean isSavingPlus(ApplicationGroupDataM appGroup)
	{
		String PRODUCT_K_PERSONAL_LOAN = SystemConstant.getConstant("PRODUCT_K_PERSONAL_LOAN");
		
		ArrayList<ApplicationDataM> applications = appGroup.filterApplicationLifeCycle(PRODUCT_K_PERSONAL_LOAN);
		if(!Util.empty(applications))
		{
			for(ApplicationDataM applicationItem : applications)
			{
				return MConstant.FLAG.ACTIVE.equals(applicationItem.getSavingPlusFlag());
			}
		}
