...
public class CISCustomerDataM implements Serializable, Cloneable{
	...
	private ArrayList<CISelcAddressDataM>  elcAddresses;
	private ArrayList<CISAddressDataM> addresses;
	...
	public ArrayList<CISelcAddressDataM> getElcAddresses() {
		return elcAddresses;
	}
	public void setElcAddresses(ArrayList<CISelcAddressDataM> elcAddresses) {
		this.elcAddresses = elcAddresses;
	}	
	...
	public CISAddressDataM selectCISAddresType(String addressType){
		if(null!=addresses && addresses.size()>0 && null!=addressType && !"".equals(addressType)){
			for(CISAddressDataM addr : addresses){
				if(addressType.contains(addr.getAddressType())){
					return addr;
				}
			}
		}
	...
	public CISelcAddressDataM selectCISelcAddresType(String typeCode,String location){
		if(null==location){
			location="";
		}
		if(null!=elcAddresses && elcAddresses.size()>0 && null!=typeCode && !"".equals(typeCode)){
			for(CISelcAddressDataM elcAddr : elcAddresses){
				String cisLocation =elcAddr.getElcAdrLoUseTpCd();
				if(null==cisLocation){
					cisLocation ="";
				}
				String cisTypeCode = elcAddr.getElcAdrUseTpCd();
				if(null==cisTypeCode){
					cisTypeCode="";
				}
				if(location.equals(cisLocation) && typeCode.equals(cisTypeCode)){
					return elcAddr;
				}
			}
		}
	...
	public CISelcAddressDataM selectCISelcAddresTypeByPhoneNo(String typeCode,String location,String phoneNo, String phoneExt){
		if(null==location){
			location="";
		}
		if(null!=elcAddresses && elcAddresses.size()>0 && null!=typeCode && !"".equals(typeCode)){
			for(CISelcAddressDataM elcAddr : elcAddresses){
				...
				if(location.equals(cisLocation) && typeCode.equals(cisTypeCode) 
						&& (phoneNo.equals(elcAddr.getTelNo()) && (phoneExt != null && phoneExt.equals(elcAddr.getTelExnNo()) || phoneExt == null))){
					return elcAddr;
				}
			}
		}
