...
public class RejectEmailPDFTemplateThai extends TemplateBuilderHelper{
	private static transient Logger logger = Logger.getLogger(RejectEmailPDFTemplateThai.class);
			 ...
			 HashMap<String, Object> hMapping = new HashMap<String, Object>();
				 ...
				 if(!InfBatchUtil.empty(rejectTemplateVariable)){
					 ...
					 if(!InfBatchUtil.empty(rejectTemplateVariables)){
						 ...
						 if(!InfBatchUtil.empty(businessClassId)){
							 ...
							 if(!InfBatchUtil.empty(productType)){
								 ...
								 if (!InfBatchUtil.empty(product) && product.equals(REJECT_LETTER_PDF_PRODUCT_NAME_KPL)) {
									 ...
									 logger.debug("Send to =>"+rejectTemplatePDFDataM.getSendTo());
									 ...
									 hMapping.put(TemplateBuilderConstant.TemplateVariableName.SEND_TO, rejectTemplatePDFDataM.getSendTo());
								 }
							 }
						 }
						 ...
						 hMapping.put(TemplateBuilderConstant.TemplateVariableName.CUSTOMER_NAME, RejectLetterPDFUtil.generatePDFCustomerName(rejectTemplatePDFDataM.getSendTo(), rejectTemplateVariable.getPersonalNameTh(), rejectTemplateVariable.getSellerCustomerNameTh()));
