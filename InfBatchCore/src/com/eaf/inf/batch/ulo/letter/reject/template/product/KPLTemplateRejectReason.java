...
public class KPLTemplateRejectReason extends RejectTemplateReasonProductHelper {
	private static transient Logger logger = Logger.getLogger(KPLTemplateRejectReason.class);
	...
	@Override
	public ArrayList<RejectLetterProcessDataM>   getTemplateRejectProduct(Object object) {
		 try {
			 ...
			 ArrayList<TemplateReasonCodeDetailDataM> templateRejectReasonDetails= GenerateLetterUtil.getLetterConditionCase(rejectLetterDataM, REJECT_LETTER_PRODUCT_NAME_KPL,isInfiniteWisdomPremierApplication,dao);
			  			 
			 if(null!=templateRejectReasonDetails && templateRejectReasonDetails.size()>0){
				 String sendTo = rejectLetterDataM.getSendTo();
				 String className = InfBatchProperty.getInfBatchConfig("REJECT_LETTER_CONDITION_SEND_TO_"+sendTo);
				 RejectLetterSendingInf sendInf = (RejectLetterSendingInf)Class.forName(className).newInstance();
//				 logger.debug("Generate templateRejectReasonDetails >>"+new Gson().toJson(templateRejectReasonDetails));
				 ...
				 if(!InfBatchUtil.empty(hReasonTempateCodeDataList)){
					 ...
					 for(String templateKey : templateKeys){
						 ...
						 rejectLetterProcessDataM.setProduct(REJECT_LETTER_PRODUCT_NAME_KPL);
						 ...
						 if(RejectLetterUtil.EMAIL.equals(letterType) || RejectLetterUtil.EMAIL_TO_SELLER.equals(letterType)){
							 rejectLetterProcessDataM.setEmail(true);
						 }else{
							 rejectLetterProcessDataM.setPaper(true);
						 }
						 if(RejectLetterUtil.EMAIL_TO_SELLER.equals(letterType)){
							 letterType = RejectLetterUtil.EMAIL;
							 rejectLetterProcessDataM.setSendSellerNoCust(true);
						 }else{
							 rejectLetterProcessDataM.setSendSellerNoCust(false);
						 }
	...
	/* @Override
	public RejectLetterProcessDataM getTemplateRejectProduct(Object object) {
		 RejectLetterProcessDataM rejectLetterProcessDataM = new RejectLetterProcessDataM();
		 try {
			 RejectLetterDataM rejectLetterDataM = (RejectLetterDataM)object;
			 String letterConditionCase = GenerateLetterUtil.getLetterConditionCase(rejectLetterDataM, REJECT_LETTER_PRODUCT_NAME_KPL);
			 logger.debug("letterConditionCase : "+letterConditionCase);
			 if(null!=letterConditionCase && !"".equals(letterConditionCase)){
				 int minRank = rejectLetterDataM.getMinRankReasonCodeProduct(REJECT_LETTER_PRODUCT_NAME_KPL);
				 TemplateReasonCodeDataM kplTemplateReason = rejectLetterDataM.getRankTemplateReasonCodeProduct(REJECT_LETTER_PRODUCT_NAME_KPL, minRank);
				 RejectLetterDAO dao = RejectLetterFactory.getRejectLetterDAO();
				 String templateCode = dao.getTemplateCode(kplTemplateReason.getPolicyRulesReasonCode());
				 String applicationType = kplTemplateReason.getApplicationType();
				 logger.debug(">>kecTemplateReason.getBusinessClassId()>>"+kplTemplateReason.getBusinessClassId());
				 logger.debug(">>applicationType>>"+applicationType);
				 logger.debug(">>templateCode>>"+templateCode);
				 if(!InfBatchUtil.empty(templateCode)){
					 rejectLetterProcessDataM.setApplyType(RejectLetterUtil.getLetterTemplateAppyType(applicationType,kplTemplateReason.getBusinessClassId()));
					 rejectLetterProcessDataM.setLanguage(kplTemplateReason.getLanguage());
					 rejectLetterProcessDataM.setProcessObject(kplTemplateReason);
					 rejectLetterProcessDataM.setProduct(kplTemplateReason.getProduct());
					 rejectLetterProcessDataM.setReasonCode(kplTemplateReason.getPolicyRulesReasonCode());
					 rejectLetterProcessDataM.setLetterConditionCase(letterConditionCase);
					 rejectLetterProcessDataM.setTemplateCode(templateCode);
				 }
			 }		 
		 } catch (Exception e) {
		...
		return rejectLetterProcessDataM;
	}*/
}
