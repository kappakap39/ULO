...
public class BundleAllAppTemplateRejectReason extends RejectTemplateReasonProductHelper{
	private static transient Logger logger = Logger.getLogger(BundleAllAppTemplateRejectReason.class);
	...
	public ArrayList<RejectLetterProcessDataM> getTemplateRejectProduct(Object object)throws Exception{
		try {
			 ...
			 ArrayList<TemplateReasonCodeDetailDataM> templateRejectReasonDetails= GenerateLetterUtil.getLetterConditionCaseBundle(rejectLetterDataM, product,isInfiniteWisdomPremierApplication,dao);
			 if(null!=templateRejectReasonDetails && templateRejectReasonDetails.size()>0){
				 ...
				 if(!InfBatchUtil.empty(hReasonTempateCodeDataList)){
					 ...
					 for(String templateKey : templateKeys){
						 ...
						 rejectLetterProcessDataM.setProduct(product);
