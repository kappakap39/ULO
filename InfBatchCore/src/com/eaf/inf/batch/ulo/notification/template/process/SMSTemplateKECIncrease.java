...
public class SMSTemplateKECIncrease extends TemplateBuilderHelper{
	
	private static transient Logger logger = Logger.getLogger(SMSTemplateKECIncrease.class);
	@Override
	public TemplateVariableDataM getTemplateVariable() {
		//TEMPLATE {PRODUCT_NAME} {CREDIT_LIMIT}
...
//			logger.debug(">>GET TEMPLATE VARIALBE<< By condition ="+ new Gson().toJson(receipientInfos));
			if(!InfBatchUtil.empty(receipientInfos)){
				HashMap<String, Object> hData = dao.getSMSTemplateKECIncrease(receipientInfos,templateMaster);
