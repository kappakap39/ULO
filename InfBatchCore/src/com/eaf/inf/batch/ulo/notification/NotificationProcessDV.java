...
public class NotificationProcessDV extends NotifyHelper{


	private static transient Logger logger = Logger.getLogger(NotificationProcessDV.class);
		...
		if(!InfBatchUtil.empty(notificationInfo)){
			...
			if(!InfBatchUtil.empty(sendTos)){
				for(String sendTo :sendTos){
					NotificationActionInf notification = NotificationControl.getNotification(PREFIX_CONFIG_NAME+"DV"+sendTo);
				...
				for(SMSRecipientDataM sms : listSmsRecipients){
					...
					if(null!=recipientinfos){
						for(RecipientInfoDataM recipientinfo :recipientinfos){
							...
							String className = InfBatchProperty.getInfBatchConfig("NOTIFICATION_REALTIME_DV_NOTIFY_TEMPLATE_"+templateSendTo);
