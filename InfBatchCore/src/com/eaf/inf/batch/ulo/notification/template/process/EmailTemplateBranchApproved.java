...
public class EmailTemplateBranchApproved extends TemplateBuilderHelper{
	private static transient Logger logger = Logger.getLogger(EmailTemplateBranchApproved.class);
	@Override
...
//			logger.debug(">>GET TEMPLATE VARIALBE<< By condition ="+ new Gson().toJson(receipientInfos));
			if(null!=receipientInfos && receipientInfos.size()>0){
				RecipientInfoDataM recipientInfoDataM = receipientInfos.get(0);
				HashMap<String, Object> hData = dao.getEmailBranchApprovedValues(recipientInfoDataM.getApplicationGroupId(),recipientInfoDataM.getMaxLifeCycle());
