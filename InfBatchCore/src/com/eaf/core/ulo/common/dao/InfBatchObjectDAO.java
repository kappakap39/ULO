...
public class InfBatchObjectDAO {
	private static transient Logger logger = Logger.getLogger(InfBatchObjectDAO.class);
	public static final Connection getConnection() {
		try {
			InfBatchServiceLocator infBatchService = InfBatchServiceLocator.getInstance();
			return infBatchService.getConnection(InfBatchServiceLocator.ORIG_DB);
		} catch (Exception e) {
	...
	public static final Connection getConnection(int dbType) {
		try {
			InfBatchServiceLocator infBatchService = InfBatchServiceLocator.getInstance();
			return infBatchService.getConnection(dbType);
		} catch (Exception e) {
			logger.fatal("ERROR", e);
		}
		return null;
	}
	public final static void closeConnection(Connection conn) throws Exception {
		try {
			if (null != conn) {
				conn.close();
			}
	...
	public static final void closeConnection(Connection con, ResultSet rs, PreparedStatement ps)
			throws Exception {
		try {
			if (rs != null) {
				try {
	...
	public final String genSqlInClauseCondition(String fieldName, Vector<?> stringOfCondition) {
		...
		if (stringOfCondition != null && stringOfCondition.size() > 0) {
			...
			for (int i = 0; i < stringOfCondition.size(); i++) {
				...
				if (((i + 1) % 800) == 0) {
					totalInClz.append("(" + fieldName + " IN ("
							+ inClz.substring(0, inClz.length() - 2) + ")) OR ");
	...
	public void setInteger(PreparedStatement ps, int index, String number) throws SQLException {
		Integer value = null;
		try {
	...
	public static void PreparedStatementParameter(int index, PreparedStatement ps,
			Vector<String> param) throws SQLException {
		if (null != param && param.size() > 0) {
			for (int i = 0; i < param.size(); i++) {
	...
	public final void closeConnection(Connection con, ResultSet rs, CallableStatement cs)
			throws Exception {
		try {
			if (rs != null) {
				try {
