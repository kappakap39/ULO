package com.eaf.core.ulo.service.notify.controller;
...
public class NotifyController {
	private static transient Logger logger = Logger.getLogger(NotifyController.class);
	public static NotifyResponse notify(NotifyRequest notifyRequest, ArrayList<String> templateList){
		logger.debug("NotifyController.notify >> "+notifyRequest);
		NotifyResponse notifyResponse = new NotifyResponse();
		ArrayList<TransactionLogDataM> transactionLogs = new ArrayList<TransactionLogDataM>();
		try{
			ArrayList<NotifyTemplateDataM> sendNotifyTemplate = new ArrayList<NotifyTemplateDataM>();
			notifyResponse.setNotifyTemplateList(sendNotifyTemplate);
			...
			if(null != notify && requiredNotify){
				...
				notify.init(notifyRequest,recipient,transactionLogs);		
						 ...
						 if(null != transactions){
							 for (NotifyTransactionResultDataM transactionResult : transactions) {
								 try{								 
									...
									sendNotifyTemplate.addAll(notifyTemplates);
									...
									if(!InfBatchUtil.empty(notifyTemplates)){
										for (NotifyTemplateDataM notifyTemplate : notifyTemplates) {
											...
											//new logic for check template that want to send
											if(!InfBatchUtil.empty(templateList) && !templateList.contains(templateType)){
												continue;
											}
...
//									}
			...
			notify.notifyLog(notifyRequest,notifyResponse);
		}catch(Exception e){
			logger.fatal("ERROR ",e);
			notifyResponse.setStatusCode(InfBatchConstant.ResultCode.FAIL);
			notifyResponse.setStatusDesc(e.getLocalizedMessage());
