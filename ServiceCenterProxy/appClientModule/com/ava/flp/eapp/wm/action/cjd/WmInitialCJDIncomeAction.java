package com.ava.flp.eapp.wm.action.cjd;
...
public class WmInitialCJDIncomeAction extends ProcessActionHelper {

	private Logger logger = Logger.getLogger(WmInitialCJDIncomeAction.class);
	
	@Override
	public Object processAction() {
		logger.debug("Start WmInitialCJDIncomeAction");
		...
		try{
			...
			logger.debug("WmInitialCJDIncomeAction Start Create Request Header");
	        HttpHeaders httpHeaderReq = new HttpHeaders();
	        httpHeaderReq.add("Content-Type", "application/json; charset=UTF-8");
	
	        logger.debug("WmInitialCJDIncomeAction Start Create Request Body");
    		String endPointUrl = SystemConfig.getProperty("WORK_MANAGER_INITIAL_ACTION_URL");
		...
		logger.debug("End WmInitialCJDIncomeAction");
		return response;
	}
}
