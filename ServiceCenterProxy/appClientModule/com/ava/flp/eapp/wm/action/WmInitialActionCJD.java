...
public class WmInitialActionCJD extends ProcessActionHelper {

	private Logger logger = Logger.getLogger(WmInitialActionCJD.class);
	
	@Override
	public Object processAction() {
		logger.debug("Start WmInitialActionCJD");
		...
		try{
			...
			logger.debug("WmInitialActionCJD Start Create Request Header");
	        HttpHeaders httpHeaderReq = new HttpHeaders();
	        httpHeaderReq.add("Content-Type", "application/json; charset=UTF-8");
	
	        logger.debug("WmInitialActionCJD Start Create Request Body");
    		String endPointUrl = SystemConfig.getProperty("WORK_MANAGER_INITIAL_ACTION_CJD_URL");
		...
		logger.debug("End WmInitialActionCJD");
		return response;
	}
}
