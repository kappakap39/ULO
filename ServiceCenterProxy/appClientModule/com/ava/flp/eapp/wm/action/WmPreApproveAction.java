...
public class WmPreApproveAction extends ProcessActionHelper {

	private Logger logger = Logger.getLogger(WmPreApproveAction.class);
	
	@Override
	public Object processAction() {
		logger.debug("Start WmPreApproveAction");
	    ...
	    try{
	    	...
	    	logger.debug("WmPreApproveAction Start Create Request Header");
	        HttpHeaders httpHeaderReq = new HttpHeaders();
	        httpHeaderReq.setContentType(MediaType.APPLICATION_JSON);
	
	        Gson gson = new Gson();
	        
	        logger.debug("WmPreApproveAction Start Create Request Body");
    		String endPointUrl = SystemConfig.getProperty("WORK_MANAGER_PRE_APPROVED_ACTION_URL");
	    ...
	    logger.debug("End WmPreApproveAction");
		return response;
	}
}
