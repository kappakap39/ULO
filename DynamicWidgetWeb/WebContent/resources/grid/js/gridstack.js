...
(function (factory) {
    ...
    GridStackEngine.prototype._fix_collisions = function (node) {
        ...
        var nn = node, has_locked = Boolean(_.find(this.nodes, function (n) {
            return n.locked
        }));
    ...
    GridStackEngine.prototype.update_node = function (node) {
        node = this._prepare_node(node);
    ...
    GridStack.prototype._prepare_element = function (el) {
        ...
        var node = self.grid.add_node({
            ...
            widgetAlignment: el.attr('data-gs-widget-alignment'),
            el: el,
            // added
            title: el.attr('data-gs-title'),
            icon: el.attr('data-gs-icon'),
            // Widget
            widgetList: el.data('widgetList')
        });
        ...
        if (node.noMove || this._is_one_column_mode()) {
            el.draggable('disable');
        }

        if (node.noResize || this._is_one_column_mode()) {
            el.resizable('disable');
        }

        // Add for Dont Resize All node
        if (this.opts.dont_resize) {
            node.el.resizable('disable');
        }

        // Add for Dont Move All node
        if (this.opts.dont_move) {
            node.el.draggable('disable');
        }
    ...
    GridStack.prototype.add_widget = function (el, node, auto_position) {
        el = $(el);
        if (typeof node.id !== 'undefined')
            el.attr('data-gs-id', node.id);
        if (typeof node.posX !== 'undefined')
            el.attr('data-gs-x', node.posX);
        if (typeof node.posY !== 'undefined')
            el.attr('data-gs-y', node.posY);
        ...
        //Widget alignment
        if (typeof node.widgetAlignment !== 'undefined')
        	el.attr('data-gs-widget-alignment', node.widgetAlignment);
        else {
        	el.attr('data-gs-widget-alignment', "V");
        }
        

        if (typeof node.noResize !== 'undefined')
            el.attr('data-gs-no-resize', node.noResize);
        if (typeof node.noMove !== 'undefined')
            el.attr('data-gs-no-move', node.noMove);
        ...
        if (typeof node.title !== 'undefined') {
            el.find('.title').html(node.title);
            el.attr('data-gs-title', node.title);
        }

        // Set Body
        el.find('.widget-wrapper > .widget-box').html('');
        if (typeof node.content !== 'undefined')
            el.find('.widget-wrapper > .widget-box').html(node.content);

        // Footer
        if (typeof node.footer !== 'undefined')
            el.find('.panel-footer').html(node.footer);
        else
            el.find('.panel-footer').remove();

        // Set Icon
        if (typeof node.icon !== 'undefined') {
            el.find('.panel-heading > i.fa').addClass(node.icon);
            el.attr('data-gs-icon', node.icon);
        } else {
            // Default Icon
            //el.find('.panel-heading > i.fa').addClass('fa-comment');
            //el.attr('data-gs-icon', 'fa-comment');
        }

        if (typeof node.widgetList !== 'undefined')
            el.data('widgetList', node.widgetList);
    ...
    // update added by Norrapat
    GridStack.prototype.update_widget = function (el, node) {
        this.remove_widget(el);
        el = $(el);
        ...
        el.attr('data-gs-auto-position', null);
        ...
        // Set Icon
        if (typeof node.icon !== 'undefined') {
            el.find('.panel-heading > i.fa').removeClass().addClass('titleicon fa fa-fw ' + node.icon);
            el.attr('data-gs-icon', node.icon);
        } else {
            // Default Icon
            el.find('.panel-heading > i.fa').removeClass().addClass('titleicon fa fa-fw');
            el.attr('data-gs-icon', '');
        }
